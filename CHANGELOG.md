...
- Add typed flow hooks for tool, list, and HTTP pipelines via `FlowHooksOf` and `FlowCtxOf`.
- Introduce a cache plugin with memory and Redis backends for caching tool responses.
- Publish the `frontmcp` CLI to scaffold projects and wrap dev/build/init/doctor workflows.

### fix

- Stringify JSON request bodies generated by the OpenAPI adapter before forwarding to upstream APIs.

### docs

- Document flow customization, caching strategies, and OpenAPI adapter setup in the Mintlify guides.

### ci

- Harden release branch automation, publish workflows, and Codex documentation patching.
