...
OpenapiAdapter.init({
  name: 'my-api',
  specOrUrl: 'https://api.example.com/openapi.json',
...
OpenapiAdapter.init({
  ...
  headersMapper: (authInfo, headers) => {
    ...
    if (authInfo.user.tenantId) headers.set('x-tenant-id', String(authInfo.user.tenantId));
    return headers;
  },
});
```
...
OpenapiAdapter.init({
  name: 'billing',
  specOrUrl: 'https://my.api/openapi.json',
