...
This guide walks you through creating an npm package that FrontMCP servers can load at runtime via [`loadFrom()`](/frontmcp/servers/esm-packages). By the end, you'll have a published package with tools, resources, and prompts that any FrontMCP server can consume.
...
<AccordionGroup>
  ...
  <Accordion title="Test locally before publishing">
    Set up a local ESM server or use `loadFrom()` with a file path during development to verify your manifest is correct before publishing.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="ESM Packages Reference" icon="box-open" href="/frontmcp/servers/esm-packages">
    Full reference for loadFrom(), caching, auth, and hot-reload
  </Card>
