...
import EmployeeTimeApp from './apps/employee-time';

describe('Site Authorization', () => {
  it('should allow access to authorized site', async () => {
    const { scope } = await createTestingModule({
      apps: [EmployeeTimeApp],
    });

    const result = await scope.callTool('employee-time:list-employees', {
      siteId: 'site-A',
    }, {
      authInfo: {
        user: { id: 'user-1', sites: ['site-A', 'site-B'] },
      },
    });
  ...
  it('should deny access to unauthorized site', async () => {
    const { scope } = await createTestingModule({
      apps: [EmployeeTimeApp],
    });

    await expect(
      scope.callTool('employee-time:list-employees', {
    ...
    ).rejects.toThrow('Not authorized for site site-C');
  });

  it('should require admin for admin tools', async () => {
    const { scope } = await createTestingModule({
      apps: [EmployeeTimeApp],
    });

    await expect(
      scope.callTool('employee-time:admin-add-entry', {
        siteId: 'site-A',
        employeeId: 'emp-1',
        hours: 8,
      }, {
        authInfo: {
          user: { id: 'user-1', sites: ['site-A'], roles: ['user'] },
        },
      })
