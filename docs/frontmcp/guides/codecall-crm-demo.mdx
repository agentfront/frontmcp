...
export default class ActivitiesLogTool extends ToolContext<typeof inputSchema, typeof outputSchema> {
  async execute(input: z.input<typeof inputSchema>): Promise<z.infer<typeof outputSchema>> {
...
<Tip>
Pair this demo with the [platform-aware UI guide](/frontmcp/guides/ui-library) to style consent and troubleshooting screens, and mirror the automated coverage in `apps/e2e/demo-e2e-codecall/e2e/codecall.e2e.test.ts`.
</Tip>
...
<CardGroup cols={2}>
  <Card title="CodeCall Plugin Docs" icon="book" href="/frontmcp/plugins/codecall/overview">
    Full CodeCall plugin documentation
  </Card>

  <Card title="UI Library" icon="palette" href="/frontmcp/guides/ui-library">
  ...
  <Card title="Agent Orchestration" icon="robot" href="/frontmcp/guides/agents-orchestration">
