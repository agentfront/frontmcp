...
sidebarTitle: First Tool Test
slug: guides/testing-your-first-tool
description: Write end-to-end tests for your FrontMCP tools using @frontmcp/testing
icon: flask
keywords: ['frontmcp', 'mcp', 'model context protocol', 'guide', 'tutorial', 'how-to', 'testing tutorial', 'typescript']
---
...
```typescript title="e2e/server.e2e.spec.ts"
...
test('add tool returns correct sum', async ({ mcp }) => {
  ...
  expect(result.json()).toEqual({ value: 8 });
});
```

<Note>
A tool without an `outputSchema` that returns a plain value (a number, string or array) sends it as `{ value: … }`, so `json()` returns `{ value: 8 }`. Declare `outputSchema: 'number'` to send the bare number instead.
</Note>
...
Invalid tool input comes back as a tool result with `isError: true` and `_meta.code: "INVALID_INPUT"`, not as a JSON-RPC error, so match it with the string code. A numeric code such as `toBeError(-32601)` matches JSON-RPC errors only.
...
test('expense report prompt generates messages', async ({ mcp }) => {
  ...
  expect(result.messages[0].content.text).toContain('2024-01-01');
  expect(result.messages[0].content.text).toContain('Travel');
});
```
...
```typescript title="e2e/calculator.e2e.spec.ts"
import { test, expect } from '@frontmcp/testing';
...
test.describe('Calculator App', () => {
  test.describe('Basic Operations', () => {
    test('add works', async ({ mcp }) => {
      ...
      expect(result.json()).toEqual({ value: 3 });
    });
...
test.describe('MCP Server E2E Tests', () => {
  ...
  test.describe('Tools', () => {
    ...
    test('add tool computes correctly', async ({ mcp }) => {
      ...
      expect(result).toBeSuccessful();
      expect(result.json()).toEqual({ value: 300 });
    });
