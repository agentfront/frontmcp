...
Create a test file using the `.e2e.spec.ts` extension (not `.e2e.ts`):

```typescript title="src/main.e2e.spec.ts"
...
├── main.e2e.spec.ts           # Server-level tests
└── apps/
    └── calculator/
...
```typescript title="calculator.e2e.spec.ts"
import { test, expect } from '@frontmcp/testing';
