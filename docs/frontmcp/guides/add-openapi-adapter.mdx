...
icon: file-code
keywords: ['frontmcp', 'mcp', 'model context protocol', 'guide', 'tutorial', 'how-to', 'openapi tutorial', 'rest', 'typescript']
---
...
<AccordionGroup>
  <Accordion title="Upstream returns a redirect and the call fails">
    **This is deliberate.** The adapter sets `redirect: 'manual'` and refuses any 3xx -- and the
    status-0 `opaqueredirect` response browser runtimes return instead -- with
    `OPENAPI_REDIRECT_NOT_FOLLOWED` rather than following it.

    Following a redirect would send the request to a destination the *upstream* chose, not
    the one you configured. Only `baseUrl` is validated, so a 3xx is an unvalidated hop --
    including to an internal or cloud-metadata address. `fetch` also forwards custom headers
    across origins (it strips only `Authorization` and `Cookie`), which is exactly how this
    adapter injects API keys, so following would hand your backend credential to the redirect
    target.

    **Solutions:**
    - Point `baseUrl` at the final host, so no redirect is needed
    - Fix the upstream to serve the operation directly rather than redirecting
  ...
  <Accordion title="Tools have unexpected inputs">
    ...
    **Solution:**
    - Use `inputTransforms` to transform the input at request time. See the [full OpenAPI Adapter documentation](/frontmcp/adapters/openapi-adapter) for details.
