...
<Warning>
**Anchor FileSource paths to the tool file.** Relative `template: { file: './widget.tsx' }` paths are resolved against `process.cwd()`, not the tool source's directory. A widget at `src/tools/foo.widget.tsx` referenced as `./foo.widget.tsx` from `src/tools/foo.tool.ts` will fail with `ENOENT` at tool-call time. Use `fileURLToPath(new URL('./widget.tsx', import.meta.url))` (from `node:url`) or pass an absolute path.
</Warning>

<Info>
**For Claude-compatible `.tsx` widgets, set `resourceMode: 'inline'`.** The default `'cdn'` mode emits an esm.sh import map for React, which Claude's widget iframe blocks (the widget hangs on "Loading widget…"). `resourceMode: 'inline'` bundles React into the widget itself so it's self-contained and renders in every host.
</Info>
