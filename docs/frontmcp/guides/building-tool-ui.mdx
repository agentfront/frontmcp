...
A string your template returns is inserted as markup when it looks like HTML, so `template: (ctx) => ctx.output` renders any tags in the output. Other results are escaped: plain text is shown as text, an object as JSON inside `<pre>`, and a base64 PDF (`JVBERi…`) is embedded as script data. A value that starts with the PDF signature but is not base64 is shown as text.
