...
When no IP can be established the request falls back to a per-session bucket. It is never
pooled into one shared bucket, which would let a single client exhaust the budget for
everyone else.
