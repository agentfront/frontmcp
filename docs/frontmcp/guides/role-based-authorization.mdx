...
import ExpenseApp from './apps/expense';

describe('Authorization', () => {
  it('should hide admin tools from regular users', async () => {
    const { scope } = await createTestingModule({
      apps: [ExpenseApp],
    });

    // Mock regular user
    const tools = await scope.listTools({
      authInfo: {
        user: { id: 'user-1', roles: ['user'] },
      },
    });

    expect(tools.find(t => t.name === 'delete-expense')).toBeUndefined();
    expect(tools.find(t => t.name === 'create-expense')).toBeDefined();
  });

  it('should show admin tools to admins', async () => {
    const { scope } = await createTestingModule({
      apps: [ExpenseApp],
    });
    ...
    const tools = await scope.listTools({
      authInfo: {
        user: { id: 'admin-1', roles: ['admin', 'user'] },
      },
    });

    expect(tools.find(t => t.name === 'delete-expense')).toBeDefined();
  });
});
```
