...
| **ESM Package** | `loadFrom()` | In-process | Community or internal npm packages |
| **Remote** | `{ urlType: 'url', ... }` | HTTP proxy | External MCP servers |
...
1. **Dynamic Loading** — Import npm packages at server startup via `loadFrom()` without bundling them into your build
...
5. **Browser Support** — Same `loadFrom()` API works in browser environments with in-memory-only caching
...
<CardGroup cols={2}>
  <Card title="ESM Packages Reference" icon="box-open" href="/frontmcp/servers/esm-packages">
    Full API reference for loadFrom(), caching, auth, and configuration
  </Card>
