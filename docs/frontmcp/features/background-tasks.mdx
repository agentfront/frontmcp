...
| Serverless (AWS Lambda, Vercel Node functions)  | ⚠️ Not recommended  | —                      | The `in-process` runner is killed when the Lambda returns. The `cli` runner can't spawn detached processes from most serverless sandboxes. Use a queue worker instead. |
