...
icon: database
keywords: ['frontmcp', 'mcp', 'model context protocol', 'plugin', 'plugins', 'extension', 'cache', 'caching', 'typescript']
---
...
<Info>
  Cache entries are keyed using a **deterministic hash** of the tool name, the tool's validated input, and — by default
  — the **caller's identity**. Two calls share an entry only when all three match, so one user's response is never
  served to another. See `keyByIdentity` below to opt out for data that is identical for every caller.
</Info>
...
<ParamField path="keyByIdentity" type="boolean" default="true">
  Include the caller's identity in the cache key.

On by default. Most cached tools return something that depends on who is asking -- a
...
rather than one shared with every other identity-less caller.
  ...
  <Warning>
    Set this to `false` **only** when every caller would get byte-identical output: public
    reference data, a currency table, a static document. Anything derived from the caller's
    permissions, tenant or account must leave it on.
  </Warning>
</ParamField>
...
Caller isolation is automatic -- the key includes the caller's identity, so one tenant is never
served another's entry. A `tenantId` in the input is an ordinary field that selects _which_ data
to fetch:
...
export default class GetTenantDataTool extends ToolContext {
  async execute(input: { tenantId: string; dataType: string }) {
    // The caller's identity is already in the key; tenantId selects the data
...
<Tip>
  The cache key is a SHA-256 digest of the tool name, the validated input **and the caller's
  identity**. Identifiers in the input select what is fetched; they are not what keeps callers
  apart. Set `keyByIdentity: false` only when every caller would get byte-identical output.
</Tip>

### Session-Scoped Caching

User-specific data needs no special handling -- the caller's identity is part of the key, so each
caller gets their own entry:
...
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Key Derivation** | Deterministic hash of tool name + validated input + caller identity, so one caller is never served another's entry. `keyByIdentity: false` drops the identity component -- use it only for tools whose output is the same for everybody |
