...
The plugin's options are validated by Zod at construction time — invalid input throws synchronously rather than crashing later inside the bundle source. The full schema lives in [`src/skilled-openapi.types.ts`](https://github.com/agentfront/blob/main/plugins/plugin-skilled-openapi/src/skilled-openapi.types.ts) and is exported as `skilledOpenApiPluginOptionsSchema` for downstream tooling.
