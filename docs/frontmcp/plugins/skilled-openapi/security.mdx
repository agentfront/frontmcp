...
4. **IP check on every address**: an IP-literal host is checked directly (no DNS needed, so it also applies on Workers); a host name is **DNS-resolved once** at request start and rejected if any resolved address falls in a deny range:
   - **Always blocked**, even with `allowPrivateNetworks: true`: link-local (169.254/16, fe80::/10) — the AWS/GCP/Azure metadata services at 169.254.169.254 — the AWS IMDSv6 address `fd00:ec2::254`, and the unspecified / "this network" addresses (0/8, `::`)
   ...
   - **IPv6 forms that carry an IPv4 address** are judged by that IPv4 address: IPv4-mapped (`::ffff:0:0/96`), IPv4-translated (`::ffff:0:0:0/96`), IPv4-compatible (`::/96`), NAT64 (`64:ff9b::/96`, `64:ff9b:1::/48`) and 6to4 (`2002::/16`). Addresses are parsed before they are checked, so `https://[::ffff:169.254.169.254]` — which `new URL()` rewrites to `[::ffff:a9fe:a9fe]` — is refused in every spelling. An address that does not parse is refused.
...
- **Structural separation**: upstream responses never reach the LLM directly. Each `callTool` returns the action's `data` _into the sandboxed AgentScript_, and only the script's final `return <value>` (surfaced as `run_workflow`'s `{ success, value, error, stats }`) is handed back to the LLM. Raw response bodies are opaque to the model unless the script explicitly extracts and returns them.
