...
```
tools/list:
  - search_skill, load_skill, run_workflow     # from the plugin
...
Step 2 is a small script you run in CI — read your OpenAPI spec, group operations into skills, and emit a signed `ResolvedBundle`.
...
The adapter writes to `scope.tools`; the plugin only ever writes meta-tool names (`search_skill`, `load_skill`, `run_workflow`) to `scope.tools`. Your adapter-mounted ops and the meta-tools cannot collide unless you deliberately name an adapter tool `search_skill`.
