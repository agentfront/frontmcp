...
<CardGroup cols={2}>
  ...
  <Card title="Execution Gate" icon="shield">
    Blocks direct `tools/call`, `resources/read` and `prompts/get` that bypass the list filter — no sneaking past disabled flags
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Capability Filtering">
    Capabilities with disabled flags are filtered out before reaching the client. Execution gates also block direct `tools/call`, `resources/read` and `prompts/get` for disabled flags, so a flag withholds the capability rather than merely hiding it from the menu.
  </Step>
</Steps>
...
Add `featureFlag` to any tool, resource, prompt, or skill metadata. Capabilities with disabled flags are hidden from `list` responses **and** refused on direct access -- `tools/call`, `resources/read` and `prompts/get` all check the flag before executing. A client that cached an earlier listing, or holds a URI from a previous session, gets an error rather than the capability.
