...
Approvals are looked up by the tool's full name, `<app id>:<tool name>` (for example `my-app:file_write`), so
pass that name to the `this.approval` grant and check methods.
