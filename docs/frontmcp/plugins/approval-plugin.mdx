...
icon: shield-check
keywords: ['frontmcp', 'mcp', 'model context protocol', 'plugin', 'plugins', 'extension', 'approval', 'consent', 'typescript']
---
...
provide, against its own store. It never judges another app's tools, so two apps can each install it with separate
stores. Installed on the server (`@FrontMcp({ plugins })`), it gates every tool. A tool both gate must pass each
store's check, and a denial in either one refuses the call.

<Warning>
  With two apps that each install `ApprovalPlugin` with its own store, `this.approval` currently resolves the store
  of the app registered last, whichever app the tool belongs to
  ([#600](https://github.com/agentfront/frontmcp/issues/600)). Until that is fixed, grant approvals through each
  app's store directly, or install `ApprovalPlugin` once on the server.
</Warning>

Session approvals belong to the caller's session. On the stateless HTTP transport every request carries the
same session id, so there they are keyed by the authenticated principal instead: `authInfo.extra.userId`,
then `authInfo.extra.sub`, then `authInfo.clientId` (the token's `sub`). A stateless call with no authenticated principal cannot hold a session approval, and a tool that
requires approval stays refused for it.
