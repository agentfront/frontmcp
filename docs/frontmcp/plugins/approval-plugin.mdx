...
same session id, so there they are keyed by the authenticated principal (`authInfo.clientId`, the token's `sub`)
instead. A stateless call with no authenticated principal cannot hold a session approval, and a tool that
