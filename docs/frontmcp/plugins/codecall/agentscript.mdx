...
(same rule as [`callTool`](/frontmcp/plugins/codecall/security#self-reference-guard)), a tool
outside the `allowedTools` whitelist the script was invoked with, or a schema that has no plain
JSON form. Always guard the result before reading from it:

```ts
const meta = getTool('users:list');
if (!meta) return { skipped: 'users:list is not available to this script' };
