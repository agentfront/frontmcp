...
The patterns are matched against **every name a tool answers to** — both its bare `name` and its
qualified `fullName` — so a tool reachable as either `wipeConfig` or `system:wipeConfig` is blocked
under both spellings.

The same policy decides discovery and execution. A tool it denies is absent from CodeCall search
results _and_ rejected at `callTool`; hiding a tool from search is not authorization on its own
(GHSA-6w3j-82v5-6qrr).
