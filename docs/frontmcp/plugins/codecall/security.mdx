...
VM presets control runtime limits (timeout, max iterations, max tool calls, console output budget). The full preset reference and Enclave Security Level mapping live in the [Configuration page → VM Presets section](/frontmcp/plugins/codecall/configuration#vm-configuration). Use `preset: 'secure'` for production unless you have a specific reason to deviate.
