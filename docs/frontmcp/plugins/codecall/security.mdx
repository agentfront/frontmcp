...
icon: shield
keywords: ['frontmcp', 'mcp', 'model context protocol', 'plugin', 'plugins', 'extension', 'codecall', 'agentscript', 'code execution', 'security', 'typescript']
---
...
[AST Guard](https://github.com/agentfront/enclave/tree/main/libs/ast-guard) parses JavaScript into an Abstract Syntax Tree and validates every node against security rules before any code executes.
...
| **`for` loops**           | Allowed                                                                                                                       | Bounded iteration with `maxSteps` enforcement |
| **`for-of` loops**        | Allowed                                                                                                                       | Bounded by array length + `maxSteps`          |
...
| `for (...)`        | Iteration-limited version            | Enforce maxSteps       |
...
[Enclave](https://github.com/agentfront/enclave) executes transformed code in an isolated Node.js `vm` context.
...
| `maxSteps`              | 5,000   | Maximum loop iterations                       |
...
VM presets control runtime limits (timeout, max steps, max tool calls, console output budget). The full preset reference and Enclave Security Level mapping live in the [Configuration page → VM Presets section](/frontmcp/plugins/codecall/configuration#vm-configuration). Use `preset: 'secure'` for production unless you have a specific reason to deviate.
...
CodeCallPlugin.init({
  includeTools: (tool) => {
    ...
    // Block tools tagged as destructive
...
`tool.name` is the tool's own name (`admin:deleteUser`), never the app-qualified `crm:admin:deleteUser`,
and the filter receives the same object wherever CodeCall evaluates it. Filter by app with `tool.appId`, which
names the owning app for the tools its adapters and plugins provide as well as for the tools it declares.
...
An entry matches a tool's own name or its qualified `<appId>:<name>`; everything else is refused. The
allowlist narrows the base policy and never widens it, so listing a tool that `includeTools` or
`enabledInCodeCall: false` withholds does not make it callable.
...
`codecall-tool-policy.ts` blocks these patterns by default, whatever the configuration says:
...
The patterns are matched against **every name a tool answers to** — its bare `name`, its
qualified `fullName` and the spelling the caller used — so a tool reachable as either `wipeConfig` or
`system:wipeConfig` is blocked under both spellings. Tools hidden from discovery
(`visibility: 'hidden'` or `hideFromDiscovery: true`) are withheld too, and so are `visibility: 'internal'`
tools, which refuse every external `tools/call` and so are never searchable, describable or bound in a script.

### One Policy for Every Surface

The same policy, evaluated on the same tool object, decides every CodeCall surface
(GHSA-6w3j-82v5-6qrr):

| Surface                          | A tool the policy withholds is…                            |
| -------------------------------- | ---------------------------------------------------------- |
| `codecall:search`                | not indexed                                                |
| `codecall:describe`              | reported in `notFound`, exactly like an unknown name       |
| `getTool()` in a script          | `undefined`                                                |
| Namespace bindings (`ns.method`) | not bound                                                  |
| `callTool()` in a script         | refused with `ACCESS_DENIED` before `tools:call-tool` runs |
| `codecall:invoke`                | refused; `directCalls` can refuse more, never less         |

Hiding a tool from search is not authorization on its own; the refusal at execution is what enforces it.
