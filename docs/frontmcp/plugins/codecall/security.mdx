...
and the filter receives the same object wherever CodeCall evaluates it. Filter by app with `tool.appId`.
...
(`visibility: 'hidden'` or `hideFromDiscovery: true`) are withheld too.
