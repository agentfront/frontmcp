...
icon: gear
keywords: ['frontmcp', 'mcp', 'model context protocol', 'plugin', 'plugins', 'extension', 'codecall', 'agentscript', 'code execution', 'configuration', 'typescript']
---
...
interface IncludeToolsFilterToolInfo {
  ...
  appId?: string;        // the owning app's id, when an app owns the tool
...
| Preset         | Timeout | Max Steps | Console | Use Case                     |
| -------------- | ------- | --------- | ------- | ---------------------------- |
