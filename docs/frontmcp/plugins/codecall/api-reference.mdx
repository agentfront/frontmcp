...
icon: book
keywords: ['frontmcp', 'mcp', 'model context protocol', 'plugin', 'plugins', 'extension', 'codecall', 'agentscript', 'code execution', 'api', 'reference', 'typescript']
---
...
```json
{
  ...
  "notFound": ["string (optional) - Tool names that weren't found, or that the access policy withholds"]
}
```
...
| `ACCESS_DENIED`          | `tool_error`     | Tool withheld by the access policy, or not in the script's `allowedTools` | Use a tool `codecall:search` returns |
