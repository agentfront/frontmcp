...
Ten minutes after the instance starts, on a timer, never on the request path.

The delay is the point. An instance that starts mid-rollout shares the store with the
instances it is replacing, and those still read and write the legacy prefixes -- the `v2:`
segment protects this version's data, not theirs. Waiting out a normal rolling deploy is what
keeps the purge from deleting memory another instance is still serving. Raise
`legacyPurgeDelayMs` for a slower rollout:

```typescript
RememberPlugin.init({
  type: 'global-store',
  legacyPurgeDelayMs: 1_800_000, // 30 minutes
});
```
