...
`session` and `tool` entries written before this change cannot be decrypted after it. There is
no automatic migration, and the failure is **silent**: decryption returns `null`, so the value
reads as absent rather than raising.

Purge the affected keys as part of the upgrade so the state is explicit rather than
mysteriously empty:
...
`user` and `global` entries are unaffected: those scopes already derived their key from the
master secret.
