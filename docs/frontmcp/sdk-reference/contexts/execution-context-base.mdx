...
icon: "code"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'context', 'execution context', 'execution', 'base', 'typescript']
---
...
**Throws:** `ProviderNotAvailableError` (`PROVIDER_NOT_AVAILABLE`) when no registry up the hierarchy provides the token. Its message names a class token; a string or symbol token shows as `[ref]` (`Provider "[ref]" is not available: not found in local or parent registries`). Use `tryGet()` to get `undefined` instead.
...
`this.auth` reads roles and permissions through the server's `authorities.claimsMapping` when one is configured, the same mapping the authorities engine uses, on every transport (including MCP 2026-07-28 requests).
...
- Trace context headers (`traceparent`, `x-request-id`)
- The caller's `Authorization` token and the request's `x-frontmcp-*` headers, only for origins listed in [`@FrontMcp({ fetch })`](/frontmcp/extensibility/request-context#context-aware-fetch)
