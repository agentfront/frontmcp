...
icon: "layer-group"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'context', 'execution context', 'typescript']
---
...
<CardGroup cols={2}>
  ...
  <Card title="ChannelContext" icon="tower-broadcast" href="/frontmcp/sdk-reference/contexts/channel-context">
...
Contexts that extend `ExecutionContextBase` (Tool, Resource, Agent, Skill,
...
Contexts that extend `ExecutionContextBase` provide a context-aware `fetch()`
(not available on `PromptContext`):

```typescript
async execute(input) {
  // Adds trace context; the caller's token only for origins allow-listed in @FrontMcp({ fetch })
