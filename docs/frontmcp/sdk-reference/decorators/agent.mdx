...
| `llm`    | `AgentLlmBuiltinConfig \| AgentLlmAdapterConfig` | LLM provider configuration |
...
<Note>
  The `process.env` examples below are Node.js-specific. In browser environments,
  avoid shipping provider API keys in browser bundles. Prefer a short-lived token
  obtained via a backend proxy.

  ```typescript
  // Browser (recommended): fetch short-lived token from your backend
  apiKey: await fetch('/api/llm-token').then((r) => r.text()),

  // Local development only (not a secret in client bundles):
