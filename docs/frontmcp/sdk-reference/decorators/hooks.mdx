...
A hook that throws stops the flow, and the error reaches the client like an error from the stage itself. An Around hook sees a failing stage as a rejected `next()`, so it can catch, retry or rethrow:
