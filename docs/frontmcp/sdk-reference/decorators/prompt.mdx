...
icon: "message"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'decorators', 'typescript', 'prompt']
---
...
`execute()` returns a `GetPromptResult`, or a string, a message array or an object that the SDK turns into one (see [Return Values](/frontmcp/servers/prompts#return-values)). The full form is:
