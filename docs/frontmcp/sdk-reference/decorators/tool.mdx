...
| `availableWhen`  | `EnvironmentMatcher`        | Restrict discovery to specific platforms/runtimes/deployments. See [Environment Awareness](/frontmcp/features/environment-awareness) |
| `execution`      | `ExecutionOptions`          | Execution-level options (queueing, retry behavior)                                 |
| `authProviders`  | `AuthProviderMapping[]`     | Required auth providers for this tool                                              |
| `annotations`    | `ToolAnnotations`           | MCP hints (`readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`)   |
| `examples`       | `ToolExample[]`             | Usage examples surfaced through `codecall:describe`                                |
| `ui`             | `ToolUiConfig`              | UI widget configuration (template, displayMode, servingMode)                       |
...
class SensitiveTool extends ToolContext {
  async execute(input) {
    ...
    const token = this.requestContext.authInfo?.accessToken;
