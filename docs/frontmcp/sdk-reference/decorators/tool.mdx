...
icon: "wrench"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'decorators', 'typescript', 'tool']
---
...
The decorator is generic over the input shape `I` and (optionally) the output
...
```typescript
// outputSchema provided → execute()'s return type is checked against it
function Tool<I extends ZodRawShape, O>(
  opts: ToolMetadataOptions<I, O> & { outputSchema: O },
): <C>(cls: C) => C; // rewrapped with inferred ToolContext<…, In, Out>

// outputSchema omitted → execute() may return anything
...
| `outputSchema`      | `ToolOutputType`                                            | Output validation schema: a Zod schema or raw shape, a literal (`'string'`, `'number'`, `'boolean'`, `'date'`, `'image'`, `'audio'`, `'resource'`, `'resource_link'`), or an array of these for multiple content items              |
...
// Primitive outputs: 'string', 'number', 'boolean', 'date'
@Tool({
  name: 'calculate',
...
@Tool({
  ...
  outputSchema: 'string', // Returns plain text, not { value: ... }
})
...
@Tool({
  name: 'link_document',
  inputSchema: { uri: z.string() },
  outputSchema: 'resource_link', // Returns a link to an MCP resource
})
```
...
@Tool({
  ...
  authProviders: [
    { name: 'github', scopes: ['repo', 'user'] }, // required by default
  ],
})
class SensitiveTool extends ToolContext {
  async execute(input) {
    // The credential gate runs before execute(): a required provider with no
    // connected credential aborts the call with a JSON-RPC -32001 error
    // (data: { tool, providers, authUrl }) — execute() never runs in that case.
    // `scopes` are advertised via the server's PRM `scopes_supported`.
    const userId = this.auth.user.sub;
    const token = this.context.authInfo?.token; // raw bearer for outbound calls
    return await callApi(token, input.data, userId);
  }
}
```
...
@Tool({
  ...
  ui: {
    // `html` escapes interpolated values; plain string results render as markup unless `escapeStringResults: true`
    template: (ctx) => ctx.helpers.html`<div>User ${ctx.input.userId}</div>`,
  },
})
```

See the [Tool UI guide](/frontmcp/servers/tools#tool-ui) for the full `ToolUIConfig` shape (CSP, sanitization, helpers, etc.), and [Trusted markup](/frontmcp/guides/building-tool-ui#trusted-markup) for `html` / `trustedHtml` and the `escapeStringResults` option (escaping becomes the default in 1.9).
...
async execute(input) {
  // Rich, typed identity (preferred)
  const userId = this.auth.user.sub;          // subject (e.g. JWT `sub`)
  const email = this.auth.user.email;
  const tenantId = this.auth.claims['tenantId']; // raw JWT claim
  if (!this.auth.hasRole('admin')) {
    // Use the SDK error flow, not `throw new Error(...)` — a raw Error is
    // redacted to "Internal error" before it reaches the client, whereas a
    // PublicMcpError's message is surfaced. `this.fail` never returns.
    this.fail(new UnauthorizedError('Admin role required'));
  }
}
```
...
Fail with a `PublicMcpError` subclass, such as `InvalidInputError`, so the client gets its message and code. A plain `Error` is an internal error: the client gets `_meta.code: "SERVER_ERROR"`, and in production only "Internal FrontMCP error. Please contact support with error ID: err\_…".
...
async execute(input) {
  if (!input.valid) {
    this.fail(new InvalidInputError('Invalid input'));
  }
...
```typescript
import { Tool, ToolContext, App, FrontMcp, InvalidInputError } from '@frontmcp/sdk';
import { z } from '@frontmcp/sdk';
...
class CalculateTool extends ToolContext {
  async execute(input) {
    ...
    switch (input.operation) {
      ...
      case 'divide':
        if (input.b === 0) this.fail(new InvalidInputError('Division by zero'));
