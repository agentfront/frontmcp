...
icon: "file-code"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'decorators', 'typescript', 'resource', 'template']
---
...
Templates support two RFC 6570 operators:

- `{var}`: Level 1 simple expansion. It matches one path segment (no `/`), and the value is percent-encoded when the URI is built.
- `{+var}`: Level 2 reserved expansion. It can span several segments, so `files://{+path}` matches `files://docs/a/b.txt` with `path = "docs/a/b.txt"`, and reserved characters such as `/` pass through unencoded.

Other operators (`{#var}`, `{?var}`, `{&var}`, `{;var}`, `{.var}`, `{/var}`) are not supported.
...
// Several segments in one parameter
@ResourceTemplate({
  name: 'nested-file',
