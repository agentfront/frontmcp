...
- **Every failure is logged once**, with the same `errorId` the client sees, the error code, and for internal errors the original message and stack.
...
`data` carries the `errorId` and `code`. Under protocol 2026-07-28, `-32002` (resource not found) is sent as `-32602`.
