...
icon: "file"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'errors', 'exceptions', 'error handling', 'resource', 'typescript']
---
...
Thrown when reading a resource fails. This is an internal error — details are hidden from clients. A public error thrown from `execute()` or passed to `this.fail()`, such as `ResourceNotFoundError`, is not wrapped.
