...
icon: "wrench"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'errors', 'exceptions', 'error handling', 'tool', 'typescript']
---
...
<Note>
`ToolExecutionError` extends `InternalMcpError`. Clients receive a generic message with an `errorId` for support correlation, not the original stack trace. A public error thrown from `execute()`, such as `InvalidInputError`, is not wrapped: the client gets its own message and code.
</Note>
...
Thrown when a prompt's `execute()` method throws an unexpected error. This is an internal error. A public error, such as `MissingPromptArgumentError` or one passed to `this.fail()`, is not wrapped.
