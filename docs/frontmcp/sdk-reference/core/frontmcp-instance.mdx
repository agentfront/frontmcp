...
It accepts the same config as `@FrontMcp()`. The instance is built on the first request, because isolate runtimes forbid timers and I/O at module evaluation. Requests run through the same `http:request` flow as the Express host, with the same request context: the user agent, `x-frontmcp-*` headers, `traceparent` and the MCP 2026-07-28 `_meta` client info.
