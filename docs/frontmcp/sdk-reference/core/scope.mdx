...
icon: "layer-group"
keywords: ['frontmcp', 'mcp', 'model context protocol', 'sdk', 'reference', 'api', 'core api', 'instance', 'scope', 'typescript']
---
...
const tool = scope.tools.getExported('my_tool');
```
...
This is the **scope-wide** store, shared by every session. Transports must not
hand it to a session directly — wrap it with `createSessionScopedEventStore(store, sessionId)`
so a client cannot replay another session's stream with a borrowed
`Last-Event-ID` (GHSA-84j6-jc92-77jm, fixed in 1.7.2).
...
if (scope.skillSession) {
  await scope.skillSession.runWithSession(sessionId, async () => {
    const allowlist = scope.skillSession!.getToolAllowlist();
    const canUse = scope.skillSession!.isToolAllowed('my_tool');
    const activeIds = scope.skillSession!.getActiveSkillIds();
  });
}
```
