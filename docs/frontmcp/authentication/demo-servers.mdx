...
- Uses the same transport defaults documented in [Transport controls](/docs/frontmcp/servers/server#transport) so you can compare behavior with production builds.
...
Pair this with the [Transport controls reference](/docs/frontmcp/servers/server#transport) to mirror the exact transports your production server exposes.
...
<CardGroup cols={2}>
  <Card title="Authentication Modes" icon="layer-group" href="/docs/frontmcp/authentication/modes">
    Understand when to reach for public, transparent, or orchestrated auth.
  </Card>
  <Card title="Transport controls" icon="server" href="/docs/frontmcp/servers/server#transport-controls">
