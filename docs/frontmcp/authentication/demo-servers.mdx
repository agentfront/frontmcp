...
keywords: ['frontmcp', 'mcp', 'model context protocol', 'authentication', 'auth', 'oauth', 'security', 'demo', 'examples', 'typescript']
---
...
| ----------------------- | ---------------------- | ------------ | ------------------------------------------------------------------------------- | ------------------------------------------------- |
| `demo-e2e-public`       | `public`               | `3003`       | Anonymous sessions with scoped tools, Streamable HTTP + JSON fallback           | `yarn nx serve demo-e2e-public --port 3003`       |
| `demo-e2e-orchestrated` | `orchestrated` (local) | `3005`       | Stateful sessions, consent UI, incremental auth controls                        | `yarn nx serve demo-e2e-orchestrated --port 3005` |
| `demo-e2e-transparent`  | `transparent`          | `3004`       | Proxies tokens to a remote IdP while keeping local consent + transport policies | `yarn nx serve demo-e2e-transparent --port 3004`  |

<Note>
All commands assume `yarn install` has already been run at the repo root. Replace the port if it conflicts with another service.
</Note>
...
Run `yarn nx serve <project> --port <port>` and wait for the `Listening on` log. Requests are available at `http://localhost:<port>`.
