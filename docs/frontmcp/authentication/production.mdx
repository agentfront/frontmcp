...
<Tabs>
  ...
  <Tab title="Multi-Instance">
    ...
    <Info>
      **Machine ID**: Set the `MACHINE_ID` environment variable to the same value on all instances. Without it, sessions encrypted on one instance cannot be decrypted on another. See [Auth Architecture - Machine ID](/frontmcp/authentication/architecture#14-machine-id).
    </Info>
