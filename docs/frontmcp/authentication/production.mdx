...
<Steps>
  ...
  <Step title="Enable Refresh Token Flow">
    For orchestrated modes, opt into automatic upstream token refresh.
    ...
    Token TTLs are controlled by the upstream identity provider; FrontMCP does not mint its own access/refresh tokens.
