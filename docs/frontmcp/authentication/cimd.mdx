...
keywords: ['frontmcp', 'mcp', 'model context protocol', 'authentication', 'auth', 'oauth', 'security', 'cimd', 'cross-identity', 'mcp discovery', 'typescript']
---
...
enabled: true,
      ...
      cache: {
        ...
        // redis: { provider: 'redis', host: 'localhost', port: 6379 }, // required when type: 'redis'
      ...
      security: {
        ...
        // allowInsecureForTesting: false, // TEST ONLY — permits http:// CIMD URLs
      },
      ...
      network: {
        ...
        redirectPolicy: 'deny',  // 'deny' (default) | 'same-origin' | 'allow'
        maxRedirects: 5,
      },
    },
...
| `allowInsecureForTesting`  | `false`     | **Test only.** Permit `http://` (non-TLS) CIMD URLs. Never enable in production. |

### Cache Settings

| Option         | Default               | Description                                                                                    |
| -------------- | --------------------- | ---------------------------------------------------------------------------------------------- |
| `type`         | `'memory'`            | Cache backend: `'memory'` (dev/single-instance) or `'redis'` (distributed)                     |
| `redis`        | `undefined`           | Redis connection (e.g. `{ provider: 'redis', host, port }`). **Required** when `type: 'redis'` |
...
| `redirectPolicy`       | `'deny'`       | How to handle redirects when fetching the CIMD document: `'deny'` (reject any redirect), `'same-origin'` (only same-origin), or `'allow'` (follow up to `maxRedirects`) |
| `maxRedirects`         | `5`            | Maximum redirects to follow (only relevant when `redirectPolicy` allows redirects)                                                                                      |

Every redirect hop is re-validated against the SSRF rules below before it is fetched. A redirect whose target cannot be read — the status-0 `opaqueredirect` response that browser runtimes return for `redirect: 'manual'` — is refused under every `redirectPolicy`.
...
<Note>
  `redirect_uri` matching is exact, with one exception: for **loopback** redirect URIs (host `127.0.0.1`, `[::1]`, or `localhost`) the **port is ignored** when comparing the request's `redirect_uri` against the registered ones (RFC 8252 §7.3) — scheme + loopback host + path must match, but a native client's ephemeral callback port may differ. Non-loopback hosts must match exactly, port included.
</Note>
...
| `169.254.0.0/16`  | Link-local addresses (incl. cloud metadata) |
...
| `::`        | Unspecified                                        |
| `fe80::/10` | Link-local                                         |
| `fc00::/7`  | Unique local (ULA), incl. AWS IMDS `fd00:ec2::254` |
| `fec0::/10` | Site-local (deprecated)                            |
| `ff00::/8`  | Multicast                                          |

**IPv6 addresses that carry an IPv4 address** are judged by the IPv4 rules above:
...
| `64:ff9b::/96`    | NAT64 (IPv4 in the low 32 bits) |
| `2002::/16`       | 6to4                            |

The local-use NAT64 prefix `64:ff9b:1::/48` is refused outright: where it carries the IPv4 address depends on the operator's prefix length, so it cannot be decoded reliably.

Addresses are parsed before they are checked, so every spelling of a blocked address is refused: `::ffff:127.0.0.1`, `::ffff:7f00:1`, `0:0:0:0:0:ffff:7f00:1` and `[::ffff:7f00:1]` (what `new URL()` produces) are all `127.0.0.1`. A host that looks like an IP literal but does not parse is refused rather than allowed. The same checks run on every address a hostname resolves to, on each redirect hop, and on the JWKS and authorization-server metadata fetches FrontMCP makes when it verifies provider tokens.
...
<Warning>
  When CIMD validation fails at `/oauth/authorize`, FrontMCP does **not** emit an OAuth `error=`/`error_description=` redirect back to the (still-unvalidated) `redirect_uri` — doing so would be an open-redirect vector. Instead it serves a **server-rendered HTML error page** (HTTP `400` for `invalid_request`, `500` for `server_error`) at the authorization endpoint itself. There is no machine-parsable JSON OAuth error envelope for the CIMD-validation failure case; the client sees an HTML page and a non-2xx status. The error-class table above describes how the failures map to HTTP statuses and how to handle them in your **own** server-side hooks/flows (below) — not a JSON body the MCP client should parse.
</Warning>

```typescript
// MCP client handling a failed authorization request.
// On CIMD-validation failure the body is an HTML error page (not JSON),
// so branch on the HTTP status rather than attempting to parse `error.error`.
async function handleAuthorization(authUrl: string) {
  const response = await fetch(authUrl);

  if (!response.ok) {
    // 400 → invalid client_id URL / redirect_uri mismatch;
    // server-side error (e.g. 500/502) → metadata fetch/validation failed.
    console.error(`Authorization failed (HTTP ${response.status})`, await response.text());
  }
}
```
...
<CardGroup cols={2}>
  <Card title="Local OAuth" icon="key" href="/frontmcp/authentication/local">
  ...
  <Card title="Progressive Authorization" icon="layer-group" href="/frontmcp/authentication/progressive">
    Enable incremental authorization for tool-by-tool access control
  </Card>
  <Card title="Production Checklist" icon="shield-check" href="/frontmcp/authentication/production">
