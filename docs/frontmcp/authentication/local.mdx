...
| `/oauth/connect`                            | GET/POST | Mid-session add-credential flow for upstream providers  |
...
<Warning>
  DCR is for development only and is disabled in production (`NODE_ENV=production`). It accepts `redirect_uris` whose host is `localhost` or any IPv4 address (any dotted-quad, not just loopback), over `http` **or** `https`, and only the `none`, `client_secret_post`, and `client_secret_basic` auth methods. Pre-register clients for production.
</Warning>
