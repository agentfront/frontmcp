...
2. The issued token carries a `consent.selectedTools` claim (defaulting to **all available tools** — there is no interactive picker yet).
3. At call time, the token's authorized-tools claim is enforced (`isToolAuthorized`).

<Warning>
  The interactive tool-selection page is **not rendered** today, and the UI-shaped `ConsentConfig` fields have no runtime effect. See [Local OAuth → Consent](/frontmcp/authentication/local#consent-and-tool-authorization).
</Warning>
...
<Info>
  `transparent` mode is the asymmetric path: tokens are verified against the **upstream IdP's** JWKS (`providerConfig.jwksUri` / `jwks`), not a local key. `/.well-known/jwks.json` is served by `local`/`remote` mode for OAuth-discovery compatibility but does not publish asymmetric public keys.
</Info>
