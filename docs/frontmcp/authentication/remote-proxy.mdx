...
auth: {
  ...
  refresh: {
    enabled: true,
    skewSeconds: 60, // refresh upstream tokens this many seconds before expiry
  },
}
```
...
Custom claims are sourced from the upstream identity provider. Configure your IdP to include the claims you need on the upstream JWT (department, employee_id, roles, etc.); FrontMCP forwards them through `this.requestContext.authInfo`.
