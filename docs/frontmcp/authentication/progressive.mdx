...
<Warning>
  The interactive **tool-selection consent page is not rendered today.** The `consent` config drives tool-authorization *enforcement* (the authorized tools are carried as a token claim and checked at call time), defaulting to all available tools. The mock below illustrates the intended multi-app authorization experience, not current behavior. The per-provider/per-app authorization steps in the federated and incremental flows above *are* rendered. See [Local OAuth → Consent](/frontmcp/authentication/local#consent-and-tool-authorization).
</Warning>
