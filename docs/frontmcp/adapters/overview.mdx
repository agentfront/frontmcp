...
icon: circle-nodes
keywords: ['frontmcp', 'mcp', 'model context protocol', 'adapter', 'integration', 'typescript']
---
...
<Tip>
**Nx users:** Scaffold with `nx g @frontmcp/nx:adapter my-adapter --project my-app`. See [Adapter Generator](/frontmcp/nx-plugin/generators/adapter).
</Tip>
...
<CardGroup cols={1}>
  <Card title="OpenAPI Adapter" icon="code" href="/frontmcp/adapters/openapi-adapter">
    ...
    **Features:**
    ...
    - [Live polling](/frontmcp/adapters/openapi-polling) for spec changes with automatic tool rebuild
...
<Steps>
  ...
  <Step title="Join the Community">
    Visit the [FrontMCP Discussions](https://github.com/agentfront/discussions) to discover and share adapters.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Submit Your Adapter" icon="plus" href="https://github.com/agentfront/discussions">
    Created an adapter? Share it with the community! Submit a PR to add your adapter to this page.
  </Card>

  <Card title="Adapter Template" icon="code-branch" href="https://github.com/agentfront/tree/main/libs/adapters">
...
Extend `DynamicAdapter` and decorate the class with `@Adapter`. The class implements a `fetch()` method that returns tools, resources, prompts, and agents discovered at runtime.
...
export class MyCustomAdapter extends DynamicAdapter<MyApiOptions> {
  ...
  async fetch(): Promise<FrontMcpAdapterResponse> {
    ...
    // Convert the API response into FrontMCP tool / resource / prompt classes
    return {
      tools: [
        /* ... */
      ],
  ...
  // `init()` is provided by DynamicAdapter — it takes the runtime options
  // and produces a unique provider token so multiple instances can coexist.
}
```

Register one or more configured instances on a server or app:

```ts
@FrontMcp({
  info: { name: 'my-app', version: '1.0.0' },
  adapters: [
    MyCustomAdapter.init({
      name: 'production-api',
...
<CardGroup cols={3}>
  ...
  <Card title="Browse Examples" icon="code" href="https://github.com/agentfront/tree/main/apps/demo">
    See adapters in action with real examples
  </Card>
  <Card title="Join Community" icon="users" href="https://github.com/agentfront/discussions">
...
<CardGroup cols={2}>
  ...
  <Card title="Source Code" icon="github" href="https://github.com/agentfront/tree/main/libs/adapters">
  ...
  <Card title="Contributing Guide" icon="code-pull-request" href="https://github.com/agentfront/blob/main/CONTRIBUTING.md">
