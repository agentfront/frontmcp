...
- **Spec-URL redirects are not followed by default.** A 3xx from your spec host won't be chased toward a (possibly internal) target. Opt in with `loadOptions.followRedirects: true` (each hop is still re-validated on `mcp-from-openapi` &ge; 2.5.0).
...
```ts Enable External Refs (public hosts only)
...
```ts Block All External Refs (FrontMCP default)
...
<Note>
  **`followRedirects`** (a `loadOptions` field, not part of `refResolution`) defaults to `false` in FrontMCP — the spec-URL fetch will not follow 3xx redirects unless you opt in. On `mcp-from-openapi` &ge; 2.5.0 each followed hop is re-validated against the SSRF guard.
</Note>

#### `refResolution` Options Reference

These apply to **both** the spec-URL fetch and external `$ref` resolution (`mcp-from-openapi` &ge; 2.5.0).
