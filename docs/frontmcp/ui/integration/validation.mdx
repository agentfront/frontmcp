...
const result = ButtonOptionsSchema.safeParse(options);

if (!result.success) {
  return renderValidationError('button', result.error);
}
```
...
function card(content: string, options: CardOptions = {}): string {
  ...
  if (!result.success) return renderValidationError('card', result.error);
