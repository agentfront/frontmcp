...
<CardGroup cols={2}>
  <Card title="HTML Shell Builder" icon="layer-group">
    Generate self-contained HTML shells with CSP headers, data injection, and bridge runtime for MCP widgets.
  </Card>
  ...
  <Card title="React Components" icon="grid-2">
    Pre-built MUI-based components for buttons, cards, badges, alerts, forms, tables, and more with consistent styling.
  </Card>
  <Card title="Import Resolver" icon="paintbrush">
    Pluggable import resolution with esm.sh as default CDN. Resolve NPM packages for browser delivery automatically.
  </Card>
  <Card title="NPM Component Loader" icon="shield-check">
    Load components from npm, file, URL, or inline function sources with TypeScript type fetching from esm.sh.
  </Card>
  ...
  <Card title="OpenAI Resource Widgets" icon="panel-right" href="/frontmcp/ui/components/resource-widgets">
...
└───────────────────────┬─────────────────────────────────┘
                        ...
                        ▼
┌─────────────────────────────────────────────────────────┐
│                 FrontMCP UI Pipeline                     │
│  Shell → Component → Resolver                           │
...
```typescript
import { resourceWidget } from '@frontmcp/uipack';
  ...
  ui: {
    template: (ctx) => `
      <div style="border:1px solid #e2e8f0; border-radius:8px; padding:16px;">
        <h3 style="margin:0 0 4px;">${ctx.helpers.escapeHtml(ctx.output.location)}</h3>
        <p style="color:#718096; margin:0 0 12px;">Current Weather</p>
        <div style="font-size:2.5rem; margin-bottom:8px;">🌤️</div>
        <div style="font-size:1.5rem; font-weight:bold;">${ctx.helpers.escapeHtml(String(ctx.output.temperature))}°C</div>
        <div style="color:#718096;">${ctx.helpers.escapeHtml(ctx.output.conditions)}</div>
      </div>
    `,
  },
})
...
<CardGroup cols={2}>
  <Card title="Getting Started" icon="rocket-launch" href="/frontmcp/ui/getting-started">
