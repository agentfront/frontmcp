...
<Note>
**Upgrading from `autoInjectAuthHeaders`:** that context option is gone. It sent the caller's token to every origin. List an origin in `fetch.forwardCallerTokenTo` only when that service is explicitly meant to accept this server's MCP token, such as another part of your own deployment; being trusted is not enough. Every other origin gets no token: call it with `credentials: { provider }`.
</Note>
