...
`requestTimeout` applies when you pass no `signal` in the options; a `Request`'s own signal still aborts it. Inside a tool, `this.fetch()` is also aborted when the call is cancelled or its execution timeout passes.
