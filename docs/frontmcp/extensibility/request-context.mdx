...
<Note>
**Upgrading from `autoInjectAuthHeaders`:** that context option is gone. It sent the caller's token to every origin. To keep sending it to a service you trust, list that service's origin in `fetch.forwardCallerTokenTo`; every other origin gets no token.
</Note>
