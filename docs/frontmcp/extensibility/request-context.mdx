...
`requestTimeout` applies when you pass no `signal` of your own. Inside a tool, `this.fetch()` is also aborted when the call is cancelled or its execution timeout passes.
