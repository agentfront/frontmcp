...
keywords: ['frontmcp', 'mcp', 'model context protocol', 'testing', 'tests', 'jest', 'fixtures', 'matchers', 'typescript']
---
...
Check if a result is an error, optionally with a specific error code. A string is matched against the tool error code in `_meta.code`; a number is matched against a JSON-RPC error code:
...
// Tool error code: invalid input is an isError result with _meta.code "INVALID_INPUT"
expect(result).toBeError('INVALID_INPUT');

// An unknown tool is also an isError result, with _meta.code "TOOL_NOT_FOUND"
expect(await mcp.tools.call('unknown-tool', {})).toBeError('TOOL_NOT_FOUND');

// JSON-RPC error code: an unknown prompt is a JSON-RPC error
expect(await mcp.prompts.get('unknown-prompt')).toBeError(-32602);
```

Errors raised while a tool runs, including invalid input, come back as a tool result with `isError: true` and a string `_meta.code` (`INVALID_INPUT`, `NOT_FOUND`, `INTERNAL_ERROR`, …), not as a JSON-RPC error. Use the string form for them.
...
These assert on the rendered UI metadata returned by tools that ship a UI template (see [Building tool UI](/frontmcp/guides/building-tool-ui)). They run against `result._meta` / `result.content[0].text` (the rendered HTML).
...
expect(result).toBeError(true); // Wrong argument type
```
