...
keywords: ['frontmcp', 'mcp', 'model context protocol', 'testing', 'tests', 'jest', 'fixtures', 'api', 'reference', 'typescript']
---
...
```ts
mcp.tools.call(name: string, args?: Record<string, unknown>, options?: ToolCallOptions): Promise<ToolResult>
```

| Parameter               | Type                      | Description                                                                                                                |
| ----------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `name`                  | `string`                  | Tool name                                                                                                                  |
| `args`                  | `Record<string, unknown>` | Tool arguments (optional)                                                                                                  |
| `options.progressToken` | `string \| number`        | Sent as `_meta.progressToken` (optional). Defaults to a generated token while `notifications.collectProgress()` is active. |
...
interface ToolResult {
  ...
  // Content accessors
  text(): string | undefined;        // First text content
  json<T = unknown>(): T;            // Parsed JSON text (structuredContent for Tool UI results)
  hasTextContent(): boolean;
  hasImageContent(): boolean;
  hasResourceContent(): boolean;
  hasToolUI(): boolean;

  // Raw access: content items are in raw.content
  raw: CallToolResult;

  // JSON-RPC error info; a tool error is an isError result with raw._meta.code
...
interface ResourceContent {
  ...
  durationMs: number;
  text(): string | undefined;        // First text content
  json<T = unknown>(): T;            // Parsed JSON text
  mimeType(): string | undefined;    // MIME type of the first content item
  hasMimeType(type: string): boolean;
  raw: ReadResourceResult;           // Raw MCP response: URIs and blobs are in raw.contents
  error?: { code: number; message: string; data?: unknown };
}
```
...
interface PromptResult {
  ...
  raw: GetPromptResult;              // Raw MCP response
  error?: { code: number; message: string; data?: unknown };
}
...
Get the MCP protocol version the server negotiated. The client requests `2025-06-18` unless `withProtocolVersion()` sets another revision. `2026-07-28` has no `initialize` handshake, so `McpTestClient` does not support it and `build()` throws for it.
...
```ts
mcp.hasCapability(name: 'tools' | 'resources' | 'prompts' | 'logging' | 'sampling'): boolean
```
...
After `initialize`, the client opens the session's notification stream (a `GET` on the MCP endpoint), as MCP clients do. Everything the server sends there, and any notification streamed on a request's own response, is recorded.

#### notifications.collect

Return a collector over the server notifications received so far and from now on.

```ts
const notifications = mcp.notifications.collect();
notifications.received; // NotificationEntry[]: { method, params, timestamp }
notifications.has('notifications/message'); // boolean
await notifications.waitFor('notifications/message', 5000); // resolves with the entry
```

The server sends `notifications/message` (what `this.notify()` logs) only after the client sets a level:

```ts
await mcp.raw.request({ jsonrpc: '2.0', id: 1, method: 'logging/setLevel', params: { level: 'info' } });
```

#### notifications.collectProgress

Return a collector over `notifications/progress`. While it is active, `tools.call()` sends a generated `_meta.progressToken`, so `this.progress()` in the tool reports progress.

```ts
const progress = mcp.notifications.collectProgress();
await mcp.tools.call('import_files', {});
await progress.waitForComplete(5000); // until progress reaches total
progress.all; // ProgressUpdate[]: { progress, total, progressToken, timestamp }
```

Notifications on the session stream can arrive after the call's result, so wait with `waitFor()` or `waitForComplete()` before asserting on them.
...
```ts
mcp.notifications.send(method: string, params?: Record<string, unknown>): Promise<void>
```
...
```ts
auth.users: {
  admin: { sub: 'admin-001', scopes: ['admin:*', 'read', 'write', 'delete'], email: 'admin@test.local', name: 'Test Admin' };
  user: { sub: 'user-001', scopes: ['read', 'write'], email: 'user@test.local', name: 'Test User' };
  readOnly: { sub: 'readonly-001', scopes: ['read'], email: 'readonly@test.local', name: 'Read Only User' };
}
```
...
```ts
factory.createAdminToken(sub?: string): Promise<string>
```
...
```ts
factory.createUserToken(sub?: string, scopes?: string[]): Promise<string>
```
...
expect(result).toBeError(code?: number | string): void
```
...
if (message.content.type === 'text') {
  expect(message.content.text).toContain('hello');
}
```
...
```ts
expect(result).toHaveRenderedHtml(): void
expect(result).toContainHtmlElement(tag: string): void
expect(result).toBeXssSafe(): void
expect(result).toContainBoundValue(value: unknown): void
expect(result).toHaveWidgetMetadata(): void
expect(result).toHaveCssClass(className: string): void
expect(result).toHaveProperHtmlStructure(): void
expect(result).toHavePlatformMeta(platform: TestPlatformType): void
expect(result).toHaveMetaKey(key: string): void
expect(result).toHaveMetaValue(key: string, value: unknown): void
expect(result).toHavePlatformMimeType(platform: TestPlatformType): void
expect(result).toHavePlatformHtml(platform: TestPlatformType): void
```
