...
test.use({
  ...
  entryPath: '/mcp',            // Where the server mounts MCP (default: '/')
});
...
| `baseUrl`        | `string`                                 | —                   | Connect to an existing server. May also be combined with `server` to override the booted server's URL. |
| `entryPath`      | `string`                                 | `'/'`               | Where the server mounts MCP (`http.entryPath`)                                                         |
...
test.describe('against the live Twilio API', () => {
...
test('session info', async ({ mcp }) => {
  ...
  expect(mcp.protocolVersion).toBe('2025-06-18');
