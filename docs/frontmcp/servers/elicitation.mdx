...
const token = await generateElicitationToken(this.authInfo.sessionId, {
...
```ts
import { FrontMcpPlugin, PluginInterface } from '@frontmcp/sdk';
import { ElicitationRequestHook, ElicitationResultHook } from '@frontmcp/sdk/elicitation';
...
export class ElicitationAuditPlugin implements PluginInterface {
...
```ts
@FrontMcpPlugin({ name: 'elicitation-sanitizer' })
class SanitizerPlugin implements PluginInterface {
...
```ts
@FrontMcpPlugin({ name: 'elicitation-analytics' })
...
```ts
@FrontMcpPlugin({ name: 'admin-audit' })
class AdminAuditPlugin implements PluginInterface {
