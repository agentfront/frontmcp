---
title: Elicitation
description: Request structured user input from the client mid-tool-call via the MCP elicitation protocol
slug: servers/elicitation
icon: message-question
keywords: ['frontmcp', 'mcp', 'model context protocol', 'mcp server', 'server', 'components', 'elicitation', 'user input', 'typescript']
---
...
Elicitation is off by default. Enable it on the server first:
...
Without it, `this.elicit()` throws `ElicitationDisabledError`, and the call fails with `_meta.code: "ELICITATION_DISABLED"` ("Elicitation is disabled on this server.").
...
const result = await this.elicit(
  ...
  {
    mode: 'url',
    elicitationId, // Required for URL mode tracking
  }
);
```
...
<Note>
Timeouts and the single pending elicit below apply to session clients (protocol revisions before 2026-07-28), where `elicit()` waits for the answer. Under 2026-07-28, `elicit()` does not wait: the call returns `input_required`, and the entry runs again from the top when the client retries with the answer. See [Elicitation under MCP 2026-07-28](#elicitation-under-mcp-2026-07-28).
</Note>
...
## Elicitation under MCP 2026-07-28

Protocol 2026-07-28 has no server-to-client requests, so `this.elicit()` works through input requests:

1. The first call answers with `resultType: "input_required"`, the question under `inputRequests`, and a `requestState`.
2. The client retries the same call with `inputResponses` and that `requestState`.
3. The entry runs again **from the top**, and each `elicit()` it has already asked returns the answer the client sent. Code before an `elicit()` runs on every round, so keep side effects after the last `elicit()` or make them idempotent.

Capabilities come with each request, not from a session. `this.elicit()` checks the request's own capabilities, so do not use the session-based [manual capability check](#manual-capability-check) under this revision. Call `this.elicit()` directly.

Otherwise the same rules apply as in a session:

- `elicitation.enabled: true` is required.
- An accepted answer is validated against the schema. A mismatch fails the call with `INVALID_INPUT`, and an unknown `action` is rejected the same way.
- A client that did not declare the `elicitation` capability (or the requested mode) gets error `-32021` instead of a question.
- The `sendElicitationResult` fallback tool is not listed to 2026-07-28 clients.
- Anonymous callers can answer across rounds: their `requestState` is bound to one shared anonymous principal, not to the per-request anonymous id.
...
A session client's capabilities can be checked before calling `elicit()`. The check looks them up by session id, so it only applies to session clients. A [2026-07-28](#elicitation-under-mcp-2026-07-28) request has no session, so the lookup finds nothing:
...
class ConditionalElicitTool extends ToolContext {
  async execute({ action }) {
    const capabilities = this.scope.notifications.getClientCapabilities(
      this.context.authInfo?.sessionId
    );
...
This system tool is **automatically registered** for session clients (protocol revisions before 2026-07-28) that don't support elicitation. A 2026-07-28 client that does not declare the capability gets error `-32021` instead, as that revision requires. The tool:
...
class ConnectGitHubTool extends ToolContext {
  async execute() {
    // Generate an opaque correlation id for the OAuth callback
    const elicitationId = randomUUID();

    // Server-side mapping from correlation id to session
    await this.get(OAuthStateStore).set(elicitationId, {
      provider: 'github',
      sessionId: this.context.authInfo?.sessionId,
      startedAt: Date.now(),
    });
