...
keywords: ['frontmcp', 'mcp', 'model context protocol', 'mcp server', 'server', 'components', 'guard', 'rate limit', 'ast guard', 'typescript']
---
...
2. **acquireSemaphore** — Takes a slot from `throttle.globalConcurrency` and from the entity's `concurrency` (else `throttle.defaultConcurrency`). Throws `ConcurrencyLimitError` if no slot is available.
...
A request with no session (every MCP 2026-07-28 request, and stateless HTTP) is not given a fresh `'session'` bucket: it falls back to the signed-in user, and anonymous callers share one `anonymous` bucket. `'userId'` never counts an anonymous caller as a user; it falls back to the session the same way.
...
When the deadline passes, a tool's `this.signal` is aborted. Pass it to `fetch` and other cancellable work: the deadline answers the client, but it cannot stop code that ignores the signal. Until `execute()` returns, it keeps its concurrency slot, so a `maxConcurrent` limit still holds.
...
<Note>
  ...
  reachable. Use the environment variables above.
</Note>
