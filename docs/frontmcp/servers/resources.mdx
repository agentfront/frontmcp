---
title: Resources
description: Static and template resources exposed over the MCP resources protocol
slug: servers/resources
icon: book-open
keywords: ['frontmcp', 'mcp', 'model context protocol', 'mcp server', 'server', 'components', 'resources', '@Resource', 'uri', 'typescript']
---
...
<Tip>
**Nx users:** Scaffold with `nx g @frontmcp/nx:resource my-resource --project my-app`. See [Resource Generator](/frontmcp/nx-plugin/generators/resource).
</Tip>
...
class AppConfig extends ResourceContext {
...
Resource templates use **dynamic URIs** with [RFC 6570](https://tools.ietf.org/html/rfc6570) parameters: `{var}` matches one path segment, and `{+var}` (reserved expansion) can span several, as in `files://{+path}`. Other RFC 6570 operators are not supported. Parameters are extracted automatically and passed to `execute()`.
...
class UserProfile extends ResourceContext {
  execute(uri: string, params: Record<string, string>) {
    const { userId } = params;
...
class DataResource extends ResourceContext {
  execute(uri: string, params: Record<string, string>) {
    ...
    this.auth;             // FrontMcpAuthContext (typed identity helpers)
    // this.authInfo is a deprecated alias kept for backward-compat
...
class UsersList extends ResourceContext {
  async execute() {
...
class FeatureFlags extends ResourceContext {
  execute() {
    return {
...
class OrderDetails extends ResourceContext {
...
When a template returns a plain value (an object, a string, or an array of items), each content item's `uri` is based on the URI that was read, for example `orders://42`, and `orders://42#0`, `orders://42#1` for array items without their own `uri`.
...
class DocumentResource extends ResourceContext {
  async execute(uri: string, params: Record<string, string>) {
...
class GitHubRepo extends ResourceContext {
  async execute(uri: string, params: Record<string, string>) {
...
Per the [MCP spec](https://modelcontextprotocol.io/specification/2025-11-25/server/resources), both capabilities are **server-level** — they apply to all resources globally, not per-resource. There is no `subscribe` field on individual `Resource` or `ResourceTemplate` objects.

The SDK sets these capabilities automatically based on your registered resources:
...
For subscribed resources, the server sends `notifications/resources/updated` when the content changes. Resource authors can trigger this notification by calling `this.notifyUpdated()` from within a `ResourceContext`, or programmatically via `scope.resources.notifyContentChanged(uri)`.
...
`{var}` parameters are **percent-encoded** per [RFC 6570](https://datatracker.ietf.org/doc/html/rfc6570) simple expansion when a URI is built; `{+var}` parameters keep reserved characters such as `/` unencoded. This is important when parameters contain special characters like `://`, `/`, or spaces.

For example, if a tool returns a raw ID like `x-coredata://3823FD9C/ICNote/p216` and you use it as a template parameter in `notes://note/{noteId}`, the expanded URI becomes:

```
notes://note/x-coredata%3A%2F%2F3823FD9C%2FICNote%2Fp216
```
