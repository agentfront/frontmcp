...
class TracedTool extends ToolContext {
  async execute({ query }: { query: string }) {
    ...
    // Authentication (prefer over this.requestContext.authInfo)
...
<Tip>
  Use `this.requestContext.authInfo` instead of `this.requestContext.authInfo` for accessing authentication data.
...
class UpdateUserStatusTool extends ToolContext {
  async execute({ userId, status, reason }) {
    ...
    await audit.log({
      ...
      newStatus: status,
      reason,
      performedBy: this.requestContext.authInfo?.userId,
    });
