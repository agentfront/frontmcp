...
<Info>
**Set `resourceMode: 'inline'` for Claude-compatible `.tsx` widgets.** The default `resourceMode: 'cdn'` emits an esm.sh import map for React, which Claude's sandboxed widget iframe blocks (the widget hangs on "Loading widget…"). With `resourceMode: 'inline'` the bundler inlines React and ReactDOM into the widget's `<script type="module">` so the output is fully self-contained and renders in every host:

```ts
ui: {
  template: { file: widgetPath },
  resourceMode: 'inline', // bundle React inline — works in Claude
}
```
</Info>
