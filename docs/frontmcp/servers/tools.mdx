...
`tools/list` describes the input the caller sends, not the value after parsing: a field with `.default()` is not listed as required, and a field with `.transform()` is listed by its input type. The same JSON Schema is used for agents and skills.
...
```ts
// Object -> structuredContent { id, status }, text '{"id":"123","status":"created"}'
execute() {
  return { id: '123', status: 'created' };
}

// String -> structuredContent { value: 'Operation completed' }, text '{"value":"Operation completed"}'
execute() {
  return 'Operation completed';
}

// Number -> structuredContent { value: 42 }, text '{"value":42}'
execute() {
  return 42;
}
```
...
```ts
@Tool({ name: 'greet', inputSchema: { name: z.string() }, outputSchema: 'string' })
class GreetTool extends ToolContext {
  async execute({ name }: { name: string }) {
    return `Hello, ${name}!`; // text content: Hello, Alice!
  }
}
```
...
| `title`             | Human-friendly name listed by `tools/list`; omitted when not set, so `annotations.title` applies                                 |
...
@Tool({
  ...
  ui: {
    // `html` escapes every interpolated value; see "Trusted markup" in the Tool UI guide.
...
ctx.helpers.html`<p>${ctx.output.message}</p>`,
}
