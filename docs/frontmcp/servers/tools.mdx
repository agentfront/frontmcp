...
class TracedTool extends ToolContext {
  async execute({ query }: { query: string }) {
    ...
    // Authentication (prefer over this.getAuthInfo())
...
<Tip>
  Use `this.requestContext.authInfo` instead of `this.getAuthInfo()` for accessing authentication data.
...
class UpdateUserStatusTool extends ToolContext {
  async execute({ userId, status, reason }) {
    ...
    await audit.log({
      ...
      newStatus: status,
      reason,
      performedBy: this.getAuthInfo()?.userId,
    });
