---
title: Jobs
description: Background tasks with @Job decorator, queues, and scheduled execution
slug: servers/jobs
icon: briefcase
keywords: ['frontmcp', 'mcp', 'model context protocol', 'mcp server', 'server', 'components', 'jobs', 'background', '@Job', 'typescript']
---
...
<Tip>
When `jobs.enabled` is `true`, the SDK automatically registers MCP tools for job management: `list-jobs`, `execute-job`, `get-job-status`, `register-job`, and `remove-job`.
</Tip>
