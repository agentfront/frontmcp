---
title: Prompts
description: Reusable prompt templates with argument interpolation and typed inputs
slug: servers/prompts
icon: message-lines
keywords: ['frontmcp', 'mcp', 'model context protocol', 'mcp server', 'server', 'components', 'prompts', '@Prompt', 'template', 'typescript']
---
...
<Tip>
**Nx users:** Scaffold with `nx g @frontmcp/nx:prompt my-prompt --project my-app`. See [Prompt Generator](/frontmcp/nx-plugin/generators/prompt).
</Tip>
...
Prompts return messages that clients can render or send to a model. The SDK supports multiple return formats, and each one type-checks as written: a message `role` given as a string literal needs no `as const`, and `arguments` can be left out.
...
class ContextExamplePrompt extends PromptContext {
  async execute(args: Record<string, string>) {
    ...
    // Authentication
    this.authInfo;           // AuthInfo from MCP session
...
class PersonalizedGreeting extends PromptContext {
  async execute(args: Record<string, string>) {
...
class TranslatePrompt extends PromptContext {
  execute(args: Record<string, string>) {
...
class InterviewPrepPrompt extends PromptContext {
  execute(args: Record<string, string>) {
...
class ProjectSummaryPrompt extends PromptContext {
  async execute(args: Record<string, string>) {
    const db = this.get(DatabaseProvider);
