...
- **Node.js**: Version 22+ (LTS)
  - _The CLI's `doctor` command enforces Node 22 or newer; FrontMCP is developed and tested on Node 24._
- **npm ≥ 10** (or pnpm/yarn)
...
```bash Production
node dist/index.js
# Runs the compiled server (bundle produced by `npm run build`)
```
...
export default class GreetTool extends ToolContext {
  async execute(input: {
    ...
    // Access auth info (via request context)
    const userId = this.requestContext.authInfo?.user?.id;
