...
If a tool name collides with a built-in command (`login`, `logout`, `serve`, `connect`, `doctor`, `install`, `uninstall`, `resource`, `template`, `prompt`, `subscribe`, `sessions`, `daemon`), the tool subcommand is automatically suffixed with `-tool` (e.g., `login-tool`).
