...
Security headers are applied automatically by FrontMCP's HTTP middleware based on the env-driven configuration above. The header builder utilities (`readCspFromEnv`, `buildCspHeaderValue`, `getCspHeaderName`, `readSecurityHeadersFromEnv`, `applySecurityHeaders`) live in `libs/sdk/src/server/middleware/csp.middleware.ts` and are not currently re-exported from the public `@frontmcp/sdk` barrel. If you need to apply them inside a custom transport adapter, import via the source path or open a request to re-export.
