...
keywords: ['frontmcp', 'mcp', 'model context protocol', 'deployment', 'production', 'hosting', 'runtime modes', 'embedded', 'serverless', 'typescript']
---
...
The build emits a `serverless-setup.js` that sets `FRONTMCP_SERVERLESS=1` (loaded
first) plus an `index.js` entry that requires your compiled module and exposes
the handler. The Vercel and Lambda adapters compile to **CommonJS** and bundle
to `handler.cjs` via rspack:

```javascript
// Generated dist/index.js (Vercel — simplified)
...
<Tabs>
  ...
  <Tab title="Upstash (REST)">
    Upstash exposes a Vercel-KV-compatible REST API. Point the `vercel-kv`
    provider at the Upstash REST URL/token to use it from any platform:
...
| **Entry Point** | `connect()`, `connectOpenAI()`, etc. | `@FrontMcp` with `http.port` | `createHandler()` or `FRONTMCP_SERVERLESS=1` |
