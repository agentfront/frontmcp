---
title: DirectClient
description: Embed the MCP client/server in-process without going over HTTP
slug: deployment/direct-client
icon: plug
keywords: ['frontmcp', 'mcp', 'model context protocol', 'deployment', 'production', 'hosting', 'direct client', 'embedded', 'typescript']
---
...
// Connect directly using your decorated server class
...
const client = await connect({
  info: { name: 'inline', version: '1.0.0' },
...
const openaiClient = await connectOpenAI(MyServer, {
  authToken: 'my-token',
  session: { user: { sub: 'user-123' } },
});

// Claude format (tool_use blocks)
const claudeClient = await connectClaude(MyServer);

// LangChain format
const langchainClient = await connectLangChain(MyServer);

// Vercel AI format
const vercelClient = await connectVercelAI(MyServer);
```
...
```typescript
import type { ElicitationHandler } from '@frontmcp/sdk';
