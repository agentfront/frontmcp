...
A **loopback** listener is reachable only under those names, so the derived list is exact and is enforced as-is — which is precisely the DNS-rebinding case. A **routable** bind is reachable under names this process cannot enumerate, so nothing derived is enforced there until you name the public host (see below); once you do, the bound NIC address joins the list alongside it.
...
A server bound to `socketPath` has no TCP host: the socket's filesystem permissions are the boundary, and clients send an arbitrary placeholder `Host`. The *derived* allow-list is skipped there and needs no configuration — a rebound browser cannot reach a unix socket at all.

An **explicit** `allowedHosts` / `allowedOrigins` (or `FRONTMCP_ALLOWED_HOSTS`) still applies to a socket server, because it is configuration rather than a guess. Since a socket client's `Host` is a placeholder, setting one there will reject every request — leave it unset.
