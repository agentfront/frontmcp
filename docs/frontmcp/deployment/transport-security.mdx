...
A server bound to `socketPath` has no TCP host: the socket's filesystem permissions are the boundary, and clients send an arbitrary placeholder `Host`. Host checking is skipped there and needs no configuration — a rebound browser cannot reach a unix socket at all.
