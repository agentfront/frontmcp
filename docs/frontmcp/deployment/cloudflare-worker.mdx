...
`nodejs_compat_populate_process_env` mirrors `[vars]` and secrets into `process.env`; add `nodejs_compat_do_not_populate_process_env` to `wrangler.compatibilityFlags` if you deliberately want `process.env` left empty.
</Note>
