...
<img className='blog-full-image' src="../../assets/banners/mcp-run-out-of-socket.svg" alt="One server, many agents"/>
...
<img className='blog-full-image dark-img' src="../../assets/banners/frontmcp-vs-standardmcp.dark.png" alt="Before vs. after"/>
<img className='blog-full-image light-img' src="../../assets/banners/frontmcp-vs-standardmcp.light.png" alt="Before vs. after"/>
...
<Steps>
    <Step title="Lift your tools into an app">
        Wrap existing handlers with <code>@Tool</code> or <code>tool()(handler)</code> and group them
        under <code>@App</code>. Or plug in the OpenAPI adapter to generate tools from your REST API.
    </Step>
    <Step title="Choose composition">
        Start with <code>splitByApp: false</code> for a unified tool surface, then introduce
        <code>splitByApp: true</code> or standalone apps under <code>/{'{'}appId{'}'}</code> where isolation is needed.
    </Step>
    <Step title="Wire auth once">
        Decide on <b>remote</b> vs <b>local</b> OAuth. Make your tool surface depend on the logged-in user and scopes,
...
<Columns cols={2}>
    <Card title="Read the Welcome doc" href="/docs/getting-started/welcome" icon="book-open" arrow="true"
          cta="Start here">
        Get the high-level overview of FrontMCP’s concepts and how it fits into MCP.
    </Card>
    <Card title="Follow the Quickstart" href="/docs/getting-started/quickstart" icon="rocket" arrow="true"
...
<Columns cols={2}>
    <Card title="Check out the examples" href="https://github.com/agentfront/frontmcp/tree/main/apps/demo"
          icon="code-branch" arrow="true" cta="Browse demos">
        Explore example apps and tools that showcase patterns you can reuse in your own servers.
    </Card>
    <Card title="Star the repo & contribute" href="https://github.com/agentfront/frontmcp" icon="star" arrow="true"
