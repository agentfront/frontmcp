...
```ts {1,5,10}
import { FrontMcp, App, Tool } from '@frontmcp/common';
import { z } from 'zod';

@Tool({ name: 'add', description: 'Add two numbers', inputSchema: z.object({ a: z.number(), b: z.number() }) })
class AddTool { execute({ a, b }: { a: number; b: number }) { return a + b; } }

@App({ id: 'calc', name: 'Calculator', tools: [AddTool] })
class CalcApp {}

@FrontMcp({ info: { name: 'Demo 🚀', version: '0.1.0' }, apps: [CalcApp] })
...
* **App** — a logical bundle of tools via `@FrontMcpApp({...})`
