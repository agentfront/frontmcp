...
3. **Runs the model’s plan in a sandboxed VM**
   Using [`vm2`](https://github.com/patriksimek/vm2) with CSP-style presets and AST validation.
...
<picture>
  <source srcSet="../../assets/diagrams/codecall-multi-app-search.dark.png" media="(prefers-color-scheme: dark)" />
  <img
    className="blog-full-image"
    src="../../assets/diagrams/codecall-multi-app-search.light.png"
    alt="User, billing, and ops apps feeding into a FrontMCP server, with CodeCall searching across selected app IDs and returning a joined result."
  />
</picture>
...
## Inside the VM: safety, logging, and observability

CodeCall runs plans inside a `vm2` sandbox with a **CSP-like policy**.
...
- Disallowed builtins/globals (e.g. `process`, `require`, `eval`).
- Loops if `allowLoops: false`.

2. **Preset + overrides**

Presets give you an intent:

- `locked_down` – strictest; short timeouts, no loops, no console.
- `secure` – **default**; safe for untrusted LLMs.
- `balanced` – more permissive for trusted/internal code.
- `experimental` – dev-only, relaxed.

You can override any field: `timeoutMs`, `allowLoops`, `disabledGlobals`, etc.
