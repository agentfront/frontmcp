...
You have an OpenAPI spec. You want to expose it as MCP tools. You search npm, find a library that promises "OpenAPI to MCP in 5 minutes," paste your configuration, and boom—you're done.
...
// github_createRepo() → GitHubAuth → authInfo.user.githubToken
// slack_postMessage() → SlackAuth → authInfo.user.slackToken
// stripe_createCharge() → StripeAuth → authInfo.user.stripeKey
```
...
[View the test suite →](https://github.com/agentfront/frontmcp/tree/main/libs/adapters/src/openapi/__tests__)
