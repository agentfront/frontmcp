...
export default class ToolIsolationPlugin {
  ...
  async filterHooksByOwner(ctx: FlowCtxOf<'tools:call-tool'>) {
    ...
    const relevantHooks = hookRegistry.getFlowHooksForOwner(
      'tools:call-tool',
      toolOwnerId
    );
...
<CardGroup cols={3}>
  <Card title="Plugin Documentation" icon="puzzle-piece" href="/docs/servers/extensibility/plugins">
    Learn more about FrontMCP plugins
  </Card>

  <Card title="Tool Documentation" icon="wrench" href="/docs/servers/tools">
