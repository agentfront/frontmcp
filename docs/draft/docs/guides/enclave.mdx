...
Set <Code>scoringGate.scorer</Code> when you want a single scoring backend, or provide <Code>scoringGate.scoring</Code> to enable the new progressive strategy. Progressive configs automatically override <Code>scorer</Code> when both are present.

### Local LLM scoring (on-device)

`local-llm` loads a Hugging Face transformers.js pipeline so you can review scripts entirely on your infrastructure. Classification mode inspects the extracted features directly, while similarity mode pairs embeddings with VectoriaDB to compare against known-malicious patterns.

```ts
const enclaveWithLocal = new Enclave({
  scoringGate: {
    scorer: 'local-llm',
    localLlm: {
...
<Note>
  The first run downloads ~22 MB of model weights into <Code>cacheDir</Code>. Subsequent executions reuse the local cache so disconnected environments (VMs, CI) stay fast.
</Note>

<Tip>Switch <Code>mode</Code> to <Code>'similarity'</Code> and provide <Code>vectoriaConfig</Code> when you want to score scripts against a curated VectoriaDB index of risky patterns.</Tip>

### Progressive scoring (fast + detailed)

Progressive mode chains the rule-based heuristics with a second, higher-latency scorer only when needed. This keeps benign scripts under a few milliseconds while escalating suspicious ones to a local LLM or remote API.

```ts
const enclaveProgressive = new Enclave({
  scoringGate: {
    scoring: {
      ...
      detailed: {
        type: 'local-llm',
        localLlm: { modelId: 'Xenova/codebert-base', cacheDir: './.cache/transformers' },
      },
      escalationThreshold: 30,
      combination: 'max', // replace | max | avg
    },
    warnThreshold: 35,
    blockThreshold: 70,
...
Set <Code>escalationThreshold</Code> to control when the detailed scorer runs and <Code>combination</Code> to decide whether its score replaces, averages, or only bumps the fast score. If the detailed scorer cannot initialize, Enclave automatically falls back to the fast result and logs a warning.
...
| `local-llm`    | ~5-10ms  | transformers.js | Better    |
| `external-api` | ~100ms   | Network         | Best      |
| `progressive`  | ~5-100ms | Rule-based + ML | Adaptive  |
