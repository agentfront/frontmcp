...
<ParamField path="maxConsoleOutputBytes" type="number" default="1048576">
  Maximum total console output in bytes (default 1MB). Prevents I/O flood attacks via excessive logging.
</ParamField>

<ParamField path="maxConsoleCalls" type="number" default="1000">
  Maximum number of console calls allowed (default 1000). Prevents I/O flood attacks via rapid-fire logging.
</ParamField>
...
|------|-------|-------------|
| `SENSITIVE_FIELD` | 25 | Queries password/token/secret fields |
| `EXCESSIVE_LIMIT` | 20 | limit > 10,000 |
| `WILDCARD_QUERY` | 20 | query="*" or filter={} |
| `LOOP_TOOL_CALL` | 35 | callTool inside for/for-of loop |
| `EXFIL_PATTERN` | 40 | list→send or query→export sequence |
| `EXTREME_VALUE` | 15 | Numeric arg > 1,000,000 |
