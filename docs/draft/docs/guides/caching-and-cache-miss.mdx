...
## Try the Cache E2E demo

Want a fully wired playground? The `apps/e2e/demo-e2e-cache` project runs the Cache Plugin with both cached and uncached tools, an execution tracker, a prompt, and a resource so you can assert cache hits without extra glue code.

<Info>Serve it with `pnpm nx serve demo-e2e-cache --port 3016` and run the Jest suite with `pnpm nx test demo-e2e-cache`. The Nx target already sets `publicMode`, retries, and `forceExit`, so the suite is CI-ready out of the box.</Info>

```ts title="apps/e2e/demo-e2e-cache/src/apps/compute/tools/expensive-operation.tool.ts"
@Tool({
...
export default class ExpensiveOperationTool extends ToolContext {
  async execute(input: Input): Promise<Output> {
    ...
    return { ...input, result, executionCount, computedAt: new Date().toISOString() };
  }
}
```

- `CacheStatsResource` exposes `cache://stats`, letting you assert cache behavior via `mcp.resources.read()`.
- `cache-report` prompt turns execution counts into a Markdown summary so clients can visualize hit ratios.
- The Jest suite resets stats between tests and compares cached vs non-cached tools, giving you copy-ready expectations for your own services.

<Tip>Reuse the `reset-stats` tool whenever you benchmark cache tweaks locally—it clears the shared execution tracker so you always start from a clean slate.</Tip>
