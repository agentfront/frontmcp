...
| **Computed destructuring** | `const {[expr]: value} = obj`, `const {[propName()]: fn} = tool`    | Blocks runtime property name construction     |
...
Computed property names inside destructuring patterns are a common way to rebuild dangerous identifiers such as
`constructor`, `prototype`, or `__proto__` at runtime. The `NoComputedDestructuringRule` now ships inside the AgentScript preset
so models cannot sidestep identifier allowlists by concatenating strings. Add it manually only when you build a custom preset
from scratch or want a tailored error message.

```ts
import { JSAstValidator, createAgentScriptPreset, NoComputedDestructuringRule } from 'ast-guard';

const validator = new JSAstValidator([
  ...createAgentScriptPreset(),
  new NoComputedDestructuringRule({ message: 'Computed keys are never allowed in AgentScript.' }),
]);
```

```js title="Blocked"
const prop = 'constructor';
const {[prop]: Fn} = callTool;
const escape = Fn('return process')();
```

```js title="Allowed"
const { users } = await callTool('users:list', {});
const { data: result } = await callTool('reports:get', {});
```

<Warning>
  Computed keys also appear in attack chains that smuggle accessor descriptors into objects. If you rely on dynamic
  destructuring in trusted code, expose a dedicated helper instead of whitelisting this syntax for AgentScript callers.
</Warning>
