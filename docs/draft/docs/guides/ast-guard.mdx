...
Blocks computed property names in destructuring patterns so attackers can't reconstruct dangerous identifiers (`constructor`, `__proto__`, `prototype`) at runtime.

```ts
import { NoComputedDestructuringRule } from 'ast-guard';

const rule = new NoComputedDestructuringRule({
  message: 'Use static property names when destructuring tool responses.',
});
```

```js
// ❌ Blocked - recombines "constructor" to reach Function constructor
const prefix = 'con';
const {[prefix + 'structor']: ctor} = someToolResult;
const escape = ctor('return process')();
```

```js
// ✅ Allowed - uses explicit property names
const { metadata, data } = await callTool('reports:get', {});
```

<Warning>
  Computed destructuring can bypass identifier allowlists because the dangerous property name is assembled at runtime.
  Pair this rule with <Code>ReservedPrefixRule</Code> and <Code>NoCallTargetAssignmentRule</Code> for layered protection
  against runtime property attacks.
</Warning>
