...
icon: layout-grid
description: Ship HTML-first consent, status, and orchestration screens with the @frontmcp/uipack package.
---

`@frontmcp/uipack` is a platform-aware HTML component kit designed for LLM surfaces. It renders complete markup (no React runtime) and ships a Tailwind v4 theme system, HTMX hooks, page templates, and OpenAI App widgets so you can style consent screens, troubleshooting pages, or CodeCall dashboards inside any MCP server.

<Note>
HTML components live in `@frontmcp/uipack` (no React required). For React components and hooks, use `@frontmcp/ui` instead.
</Note>
...
```ts
import { baseLayout, card, button, DEFAULT_THEME } from '@frontmcp/uipack';
...
```ts
import { form, input, button } from '@frontmcp/uipack';
...
```ts
import { consentPage, consentSuccessPage, errorPage } from '@frontmcp/uipack';
...
```ts
import { button } from '@frontmcp/uipack';
