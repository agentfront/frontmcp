...
@frontmcp/ui is a platform-aware HTML component kit designed for LLM surfaces. It renders complete markup (no React runtime) and ships a Tailwind v4 theme system, page templates, and OpenAI App widgets so you can style consent screens, troubleshooting pages, or CodeCall dashboards inside any MCP server.
...
- **Bridge-ready interactivity** – Widgets stay static by default but surface `widgetAccessible` and MCP Bridge hooks for tool-driven flows.
...
## MCP Bridge-powered interactions

Widgets now render static HTML so they work on any host. Use `ui.widgetAccessible = true`, add IDs or `data-*` attributes, and wire up MCP Bridge listeners for dynamic flows.
...
<script>
  ...
  formEl?.addEventListener('submit', async (event) => {
    ...
    try {
      await window.mcpBridge.callTool('verify_login_code', data);
...
<Info>
  MCP Bridge falls back gracefully on hosts that do not expose tool calls, so keep your widgets functional even without JavaScript.
</Info>
...
<Warning>
Validation errors render in development and production. Keep an eye on your previews to ensure you only ship supported variants, attributes, and MCP Bridge handlers.
</Warning>
