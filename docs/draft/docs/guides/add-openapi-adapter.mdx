...
- A FrontMCP project initialized ([see Installation](/getting-started/installation))
...
<Steps>
  ...
  <Step title="Verify tools are loaded">
    ...
    ```
    [INFO] Generated 15 tools from expense-api
...
OpenapiAdapter.init({
  ...
  generateOptions: {
    ...
    filterFn: (op) =>
      op.path.startsWith('/invoices') ||
      op.path.startsWith('/customers'),
  },
})
```
...
<Steps>
  ...
  <Step title="Generate Tools">
    Each operation in the spec becomes an MCP tool with:
    - Automatic input schema (path params, query params, headers, body)
    - Type-safe validation using Zod
    - Description from the operation summary
  </Step>

  <Step title="Register Tools">
    Generated tools are registered with your app and inherit:
    - App-level plugins (caching, logging, etc.)
    - App-level providers
