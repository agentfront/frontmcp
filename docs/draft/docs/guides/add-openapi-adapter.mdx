...
<Info>Callbacks receive the full `FrontMcpContext`, so reach for `ctx.authInfo`, `ctx.sessionId`, or tracing data instead of legacy `authInfo` helpers.</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Authentication errors (401)">
    ...
    **Solutions:**
    ...
    - Check that `ctx.authInfo.token` contains the expected value
