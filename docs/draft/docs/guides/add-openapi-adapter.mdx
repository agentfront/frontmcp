...
- A FrontMCP project initialized ([see Installation](/docs/getting-started/installation))
...
### Hybrid Authentication (securitySchemesInInput)

When part of your OpenAPI security model needs user-provided tokens (OAuth, PATs) and the rest should stay server-side, list the caller-owned schemes in `securitySchemesInInput` and map the rest via `authProviderMapper`.
...
<Note>Only the schemes you list stay visible to MCP callers. Every other scheme must be fulfilled by `authProviderMapper`, `securityResolver`, or `staticAuth`.</Note>
...
<CardGroup cols={2}>
  <Card title="Full OpenAPI Adapter Docs" icon="book" href="/docs/adapters/openapi-adapter">
    Explore all configuration options and advanced features
  </Card>

<Card title="Authentication Setup" icon="lock" href="/docs/servers/authentication/overview">
  Learn how to configure authentication for your APIs
</Card>

<Card title="Plugin System" icon="puzzle-piece" href="/docs/servers/extensibility/plugins">
