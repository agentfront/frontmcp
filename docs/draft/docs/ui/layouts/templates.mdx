...
icon: clone
description: Pre-built layout templates for common patterns
---

# Layout Templates

FrontMCP UI includes pre-built layout templates for common widget patterns. These combine the base layout with structural components.
...
import { baseLayout, card, badge, button } from '@frontmcp/ui';
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-4 gap-2">
            ${product.images.slice(1).map(img => `
              <img src="${img}" class="rounded-lg cursor-pointer hover:opacity-75" />
            `).join('')}
          </div>
        </div>
