...
<Tip>
`helpers.escapeHtml()` now accepts any value, returns an empty string for `null`/`undefined`, and stringifies objects before escaping. You can also import `escapeHtml` directly from `@frontmcp/ui`.
</Tip>
