...
<Info>
This draft release ships both the CommonJS `main` build and an ESM `module` build (plus wildcard exports), so bundlers and Node 22+ scripts load the right format automatically without custom shims.
</Info>

<CodeGroup>
```ts CommonJS
const { card, badge } = require('@frontmcp/ui');
```
...
<Tip>Need only a specific module? Import directly from `@frontmcp/ui/components`, `@frontmcp/ui/layouts`, or any other subpath thanks to the wildcard exports.</Tip>
