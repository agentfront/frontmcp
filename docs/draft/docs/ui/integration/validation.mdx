...
<Note>
Import `validateOptions` from `@frontmcp/uipack/validation`. Utility helpers such as `escapeHtml` now live in `@frontmcp/uipack/utils`, so you can reuse them outside the built-in components.
</Note>
