...
<Note>
Use the weather demos under `apps/ui/**` as golden tests before copying patterns into your own workspace. They exercise every matcher in this guide, including `toHaveRenderedHtml`, `toNotContainRawContent`, and the `UIAssertions` helpers.
</Note>

- `apps/ui/react-demo/e2e/weather.e2e.ts` verifies the React renderer, CDN-based serving mode, and transparent auth defaults.
- `apps/ui/mdx-demo/e2e/weather.e2e.ts` ensures the MDX renderer compiles markdown + JSX without falling back to escaped content.
...
<CardGroup cols={2}>
  <Card title="Testing Library" icon="book" href="/docs/testing">
