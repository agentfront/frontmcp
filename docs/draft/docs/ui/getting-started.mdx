...
## Module formats & bundlers

`@frontmcp/ui` now ships synchronized CommonJS (`dist/index.js`) and native ESM (`dist/esm/index.js`) builds plus a single type definition tree. Node, Vite, Next.js, and TS configs automatically pick the right artifact—no custom aliases required.

```ts title="ESM / TypeScript"
import { card } from '@frontmcp/ui';
```

```ts title="CommonJS"
const { card } = require('@frontmcp/ui');
```

<Info>Both entry points share the same `index.d.ts`, so your IDE sees identical types regardless of module system.</Info>

<Warning>The legacy `@frontmcp/ui/esm` path has been removed. Import from `@frontmcp/ui` and let Node pick the correct format.</Warning>
...
## Try the Weather UI demos

<Steps>
<Step title="Spin up the React demo">

```bash
pnpm nx serve react-demo --port 3003
```

This server exposes the `get_weather` tool with a React-based widget, transparent auth defaults, and `publicMode`-friendly settings so you can hit it from the MCP Inspector.

</Step>
<Step title="Spin up the MDX demo">

```bash
pnpm nx serve mdx-demo --port 3004
```

The MDX demo renders the same weather tool with Markdown + JSX templates. Use it to compare HTML vs MDX fallbacks or to verify that custom components compile correctly.

</Step>
<Step title="Run the UI regression tests">

```bash
pnpm jest apps/ui/react-demo/e2e/weather.e2e.ts
pnpm jest apps/ui/mdx-demo/e2e/weather.e2e.ts
```

Both suites rely on `@frontmcp/testing`’s `UIAssertions` helpers (`toHaveRenderedHtml`, `toNotContainRawContent`, `assertValidUI`) so you can copy/paste the assertions into your own projects.
...
<CardGroup cols={2}>
  ...
  <Card title="Platform Support" icon="globe" href="/docs/ui/advanced/platforms">
