...
icon: moon
description: FrontMCP UI supports dark mode through theme variants and CSS media queries. Widgets can adapt to the user's system preference or platform theme.
---
...
```typescript
import { baseLayout, DEFAULT_THEME, createTheme } from '@frontmcp/uipack/components';
...
```typescript
import { card, badge, button } from '@frontmcp/uipack/components';
...
import { createTheme, DEFAULT_THEME } from '@frontmcp/uipack/components';
