...
<Note>More official plugins are coming soon! Authentication, rate limiting, observability, and validation plugins are in development.</Note>

## Server-Level Plugins

Use the `plugins` array on `@FrontMcp` to register plugins that must run once for the entire server:
...
@FrontMcp({
  info: { name: 'Ops Console', version: '1.0.0' },
  plugins: [
...
<Info>
  Server-scoped plugins execute before app-level plugins and can contribute providers/hooks that span every app, tool, resource, and agent.
</Info>

<Tip>
  Plugins that declare `scope: 'server'` must register via `@FrontMcp`. Standalone apps that include them throw `InvalidPluginScopeError`.
</Tip>
