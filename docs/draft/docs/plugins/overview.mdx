...
<CardGroup cols={1}>
  ...
  <Card title="Dashboard Plugin" icon="chart-network" href="/docs/plugins/dashboard-plugin">
    Visualize your apps, scopes, tools, and resources in an interactive dashboard that keeps itself in sync with your MCP server.

    **Features:**
    - Live graph rendered from `dashboard:graph` plus SSE updates
    - Built-in `dashboard:list-*` tools and optional auth tokens
    - CDN-first UI served from a standalone scope you can share with teammates

    **Best for:** staying confident about topology changes, onboarding new engineers, and spotting transport issues.

  </Card>

</CardGroup>

<Note>Dashboard plugin ships with `@frontmcp/plugins` so you can inspect your server instantly. More official plugins are coming soon!</Note>
