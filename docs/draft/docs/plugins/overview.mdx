...
<CardGroup cols={1}>
  <Card title="Remember Plugin" icon="brain" href="/docs/plugins/remember-plugin">
    Encrypted session memory with scoped storage, LLM-managed memory tools, and tool approval controls for secure AI agent interactions.
    ...
    **Features:**
    ...
    - Built-in memory-management tools (`memory_remember_this`, `memory_recall`, `memory_forget`, `memory_list_memories`) plus TTL/pattern validation helpers from `@frontmcp/utils`
