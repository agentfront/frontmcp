...
<ParamField path="script" type="string" required>
  JavaScript code to execute in the sandboxed VM. The script can call tools using the `call(toolName, input)` function and access results.

  Example:
  ```js
  const user = await call('users:getById', { id: 'user_123' });
  const orders = await call('orders:listByUser', { userId: user.id });
...
<ParamField path="allowedTools" type="string[]">
  ...
  ```json
  {
    "script": "const user = await call('users:getById', { id: '123' });",
...
```json
{
  "script": "const user = await call('users:getById', { id: 'user_123' }); const orders = await call('orders:listByUser', { userId: user.id }); return { userName: user.name, totalOrders: orders.length };"
}
```
...
```json
{
  "script": "const user = await call('users:getById', { id: 'user_123' }"
}
```
...
```json
{
  "script": "const user = await call('users:getById', { id: 'nonexistent' }); return user;"
}
```
...
<CardGroup cols={2}>
  ...
  <Card title="isolated-vm Documentation" icon="shield-halved" href="https://github.com/laverdet/isolated-vm">
    Official isolated-vm sandbox documentation
  </Card>
</CardGroup>
