...
Every script goes through a 6-layer security pipeline:
...
│  • Block ReDoS, BiDi attacks before parsing                     │
...
│  • Block eval, Function, dangerous globals                       │
│  • Block prototype manipulation                                  │
│  • Enforce whitelist-only identifiers                           │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
│  • Transform callTool → __safe_callTool (proxied)               │
│  • Transform loops → iteration-limited versions                  │
│  • Add resource tracking                                         │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  LAYER 3: AI SCORING GATE (Enclave)                             │
│  • Semantic analysis via AST features                            │
...
└─────────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  LAYER 4: RUNTIME SANDBOX (Enclave)                             │
│  • Standard: Node.js vm (prototype isolation)                    │
│  • Optional: Worker Pool (OS-level isolation)                    │
│    - Dual-layer sandbox (worker thread + VM)                     │
│    - Hard halt via worker.terminate()                            │
│    - Per-worker memory limits                                    │
│  • Controlled globals only (Math, JSON, Array, etc.)            │
│  • Timeout enforcement and tool call counting                    │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
