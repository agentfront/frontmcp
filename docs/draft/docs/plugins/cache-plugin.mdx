...
### Global Store (Reuse `@FrontMcp` configuration)

Best for: Deployments that already configure `redis` on `@FrontMcp` (including `provider: 'vercel-kv'`) and want every cache-enabled tool to share that store without duplicating credentials.
...
@FrontMcp({
  ...
  redis: {
    provider: 'vercel-kv', // or host/port/password for standard Redis
  },
})
...
export default class EdgeApp {}
```

<Tip>
`type: 'global-store'` automatically reuses the store defined in the `@FrontMcp` decorator—whether that's Redis or `provider: 'vercel-kv'`.
</Tip>

<Info>
Need help wiring up Vercel KV? Follow the [Vercel KV guide](/docs/deployment/vercel-kv) or pair it with Redis pub/sub using the hybrid configuration described there.
</Info>
...
<ParamField path="type" type="'memory' | 'redis' | 'redis-client' | 'global-store'" default="'memory'">
  Cache store backend to use. Set `type: 'global-store'` to reuse the `@FrontMcp`-level `redis` configuration (Redis or Vercel KV) without redefining credentials.
</ParamField>
...
<ParamField path="config" type="object">
  Redis connection configuration (required when `type: 'redis'`, ignored when `type: 'global-store'`)
...
<CardGroup cols={2}>
  <Card title="Source Code" icon="github" href="https://github.com/agentfront/frontmcp/tree/main/libs/plugins/src/cache">
