...
### Global Store (Reuse FrontMCP config)

Best for: Teams that already defined a global store on the `@FrontMcp` decorator—whether it's classic Redis or the new `provider: 'vercel-kv'`.
...
<Tip>Using `type: 'global-store'` automatically reuses whatever store you configured globally, so CachePlugin picks up Redis in self-hosted environments and Vercel KV when you deploy to Vercel—no duplicate credentials.</Tip>
...
<Note>`type: 'global-store'` ignores `config` and `client` because it reuses the store configured on `@FrontMcp` (Redis or Vercel KV). Keep `redis` defined at the server level and every CachePlugin instance can share it.</Note>
