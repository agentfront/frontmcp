...
2. This generates:
   ...
   dist/
     ...
     serverless-setup.js# Ensures FRONTMCP_SERVERLESS=1 before imports run
     index.js           # ESM entry that the bundler uses
     handler.cjs        # Single-file bundle deployed to Vercel
   vercel.json          # Vercel configuration
   ```

<Info>
Rspack bundling now happens automatically, so Vercel receives `handler.cjs` (CommonJS) while `index.js` remains available for debugging. The generated `vercel.json` already points to the bundled file.
</Info>
...
2. This generates:
   ...
   dist/
     main.js            # Your compiled server (ESM)
     serverless-setup.js# Ensures FRONTMCP_SERVERLESS=1 before imports run
     index.js           # Lambda entry before bundling
     handler.cjs        # CommonJS bundle you deploy
   ```

<Info>
Point your Lambda tooling at `dist/handler.cjs.handler`. The CLI exports the Express app from that bundle, so you get ESM-friendly builds with CommonJS compatibility.
</Info>
...
Resources:
  FrontMcpFunction:
    ...
    Properties:
      Handler: dist/handler.cjs.handler
      Runtime: nodejs20.x
      Timeout: 30
      MemorySize: 256
      Events:
...
<Note>
TypeScript still compiles to ESM inside `dist/index.js`, but the build now bundles everything into `dist/handler.cjs`. That CommonJS wrapper means you no longer need `"type": "module"` or `.mjs` handlers—just point your deployment config at `dist/handler.cjs.handler`.
</Note>
