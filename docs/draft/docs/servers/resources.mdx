...
### Function Style Templates

Prefer the builder helpers when you only need a handler function:

```ts
import { resourceTemplate } from '@frontmcp/sdk';

export const UserProfile = resourceTemplate({
  name: 'user-profile',
  uriTemplate: 'users://{userId}/profile',
  mimeType: 'application/json',
  description: 'Fetch user profile by ID',
})(async (uri, params) => {
  const profile = await fetchProfile(params.userId); // Look up data in your datastore
  return {
    contents: [
      {
        uri,
...
Function-style templates share the same metadata schema as classes, but ship zero decorator state and are ideal for simple wrappers around existing fetch helpers.
...
## Registry Lifecycle & Adoption

Every scope, app, and plugin gets its own `ResourceRegistry`. Parent scopes automatically **adopt** the resources declared by their children, so registering a resource inside an app (or plugin) instantly makes it available to the server-level `resources:*` flows.

### Inspecting resources inside a tool

```ts
import { ScopeEntry, Tool, ToolContext } from '@frontmcp/sdk';

@Tool({ name: 'resources:debug', description: 'Show available resources' })
class ResourceDebugger extends ToolContext {
  async execute() {
    const scope = this.get(ScopeEntry);
    const registry = scope.resources;

    const total = registry.getResources(true).length;
    const match = registry findResourceForUri('docs://welcome');

    return {
      totalResources: total,
      matchedResource: match?.instance?.name ?? null,
      params: match?.params ?? {},
    };
  }
}
...
### Live updates & MCP flows

- `registry.subscribe()` lets you watch for new resources (handy for custom logging or cache invalidation).
- `resources/list`, `resources/templates/list`, and `resources/read` flows pull directly from the active registry, so adopted resources travel with their lineage and metadata intact.
- Because adoption keeps references to the original instances, any provider dependencies or caches stay shared between parent and child scopes.
