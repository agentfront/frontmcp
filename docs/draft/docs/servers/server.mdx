...
<Info>
  New in v0.6.2: Defining top-level `redis` automatically enables `transport.persistence.enabled` and reuses the same connection settings unless you explicitly set `enabled: false`. This keeps persistent transports aligned with your Redis configuration without duplicating options.
</Info>
