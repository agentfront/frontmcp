...
Define DI-style singletons available to all apps (and their tools/plugins). Scopes are supported at the provider level (`GLOBAL`, `CONTEXT`). Legacy `SESSION` and `REQUEST` scopes are automatically normalized to `CONTEXT`.
