...
Transport sessions now opt into Redis persistence automatically whenever you define the top-level `redis` block. Multi-instance recovery and reconnects work out of the box—you only touch `transport.persistence` to tune TTLs or opt out.
...
@FrontMcp({
  ...
  // transport.persistence is auto-populated from redis
})
```
...
| Option         | Default                                  | Description                                        |
| -------------- | ---------------------------------------- | -------------------------------------------------- |
| `enabled`      | `auto` (true when `redis` is configured) | Enable session persistence; set `false` to opt out |
| `defaultTtlMs` | `3600000`                                | Session TTL in milliseconds (1 hour)               |

<Info>
Session persistence reuses the top-level `redis` config automatically. Override `transport.persistence` only when you need a different TTL, store, or want to disable persistence for a specific deployment.
</Info>
...
<Info>
Redis now lives in its own top-level block and automatically feeds both `transport.persistence` and `auth.tokenStorage`. You only need to touch `transport.persistence` when overriding TTLs, swapping stores, or explicitly disabling persistence.
</Info>
...
- **Session persistence** — adding `redis` turns on persistence; override `transport.persistence` only when changing TTLs or disabling it.
- **Token storage** (`auth.tokenStorage: { type: 'redis' }`) — stores refresh tokens securely with the same credentials.
...
@FrontMcp({
  redis: {
    host: 'localhost',
    keyPrefix: 'myapp:',
  }, // transport.persistence picks this up automatically
  auth: {
    mode: 'orchestrated',
    type: 'local',
...
<Info>
  ...
  when mixing public and private apps under one server umbrella. Per-app auth is required when `splitByApp: true`.
</Info>
