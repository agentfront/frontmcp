...
class DynamicPlugin<TOptions, TInput = TOptions> {
  ...
  // Accepts TInput options, returns a plugin provider
  static init(options: TInput): PluginProvider;
...
<Info>
  The `init()` method accepts your plugin's input options type and handles the internal
  wiring automatically. You don't need to import or reference internal types like
  `InitOptions` or `PluginReturn`—just pass your options object directly.
</Info>
