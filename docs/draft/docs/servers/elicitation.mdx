...
```
Tool ─► elicit() ─► Transport ─► Client UI ─► User responds ─► Result
```

**Fallback Elicitation (OpenAI, Gemini, etc.):**

```
Tool ─► elicit() ─► Framework detects no support ─► Returns instructions to LLM
                                                              │
LLM asks user ─► User responds ─► LLM calls sendElicitationResult tool
                                                              │
                                       Tool re-invoked with result ─► Completes
```
