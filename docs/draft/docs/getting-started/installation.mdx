...
- scaffold a `ci/` folder with Dockerfile, `docker-compose.yml`, and `.env.docker` when you choose the Node (Docker) target
- create an `e2e/` suite plus `jest.e2e.config.ts` powered by `@frontmcp/testing`, so `npm run test:e2e` works immediately
...
- `frontmcp test` — run your workspace tests with the same tsconfig the CLI validated
- `npm run test:e2e` — execute the generated Jest suite powered by `@frontmcp/testing`

## End-to-end tests on day one

Every scaffolded project now includes:

```
├── e2e/
│   └── server.e2e.test.ts      # Smoke tests using @frontmcp/testing
├── jest.e2e.config.ts         # Preconfigured Jest runner
└── package.json               # Adds "test" and "test:e2e" scripts
```
...
<Info>Update `test.use({ server: './src/main.ts' })` inside `e2e/server.e2e.test.ts` if you rename your entry file or need a different port.</Info>
