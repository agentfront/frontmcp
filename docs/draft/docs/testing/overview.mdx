...
## apps/e2e reference workspace

The repository now ships a dedicated `apps/e2e` workspace. Each project focuses on a specific scenario (auth, caching, CodeCall orchestration, hooks, multi-app routing, or error handling) and includes both an `nx serve` target and a Jest suite powered by `@frontmcp/testing`. Treat them as gold-standard fixtures before copy/pasting patterns into your own repo.

| Project                                                              | Scenario                                                                             | Serve                                         | Tests                            |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------- | -------------------------------- |
| `demo-e2e-public` / `demo-e2e-transparent` / `demo-e2e-orchestrated` | Public, transparent, and orchestrated auth flows with matching transport policies    | `pnpm nx serve <project> --port <port>`       | `pnpm nx test <project>`         |
| `demo-e2e-cache`                                                     | CachePlugin behaviors (tools, prompts, and resources backed by an execution tracker) | `pnpm nx serve demo-e2e-cache --port 3016`    | `pnpm nx test demo-e2e-cache`    |
| `demo-e2e-codecall`                                                  | CRM sample showcasing CodeCall search/describe/execute flows                         | `pnpm nx serve demo-e2e-codecall --port 3013` | `pnpm nx test demo-e2e-codecall` |
| `demo-e2e-hooks`                                                     | Tool lifecycle hooks plus audit logging plugin                                       | `pnpm nx serve demo-e2e-hooks --port 3018`    | `pnpm nx test demo-e2e-hooks`    |
| `demo-e2e-errors`                                                    | Public mode server that purposefully throws every MCP error variant                  | `pnpm nx serve demo-e2e-errors --port 3019`   | `pnpm nx test demo-e2e-errors`   |
| `demo-e2e-multiapp`                                                  | Multi-app calendar + tasks workspace with shared transport                           | `pnpm nx serve demo-e2e-multiapp --port 3020` | `pnpm nx test demo-e2e-multiapp` |

<Info>Each project bootstraps Redis/memory defaults, transport toggles, and TypeScript configs identical to production builds, so the e2e suites double as regression tests for the SDK.</Info>

### Example: Cache demo suite

The cache project lives at `apps/e2e/demo-e2e-cache`. It wires `CachePlugin` (memory backend, 30-second TTL) plus helper tools/resources so you can assert cache hits vs misses without writing scaffolding. The Jest suite walks through cache hits, misses, stats, prompts, and resource reads.

```ts title="apps/e2e/demo-e2e-cache/e2e/cache.e2e.test.ts"
test.use({
...
<Tip>Point `TestServer.startNx()` at any of these projects to reuse their build targets inside your own test suites. Because the Nx targets already set `forceExit`, retries, and `transformIgnorePatterns`, the fixtures stay stable even on CI.</Tip>
