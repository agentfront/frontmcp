...
interceptor.get('/users', [{ id: 1 }, { id: 2 }]);
```
...
// Match any user ID
interceptor.get(/\/users\/[a-z0-9-]+/, { id: 'matched' });
```
...
```ts
interceptor.get('/api/data', { result: 'get' });
interceptor.post('/api/data', { result: 'post' });
interceptor.put('/api/data', { result: 'put' });
interceptor.delete('/api/data', { result: 'delete' });

// Match any method
interceptor.any('/api/data', { result: 'any' });
```
...
test('verify HTTP calls', async ({ mcp }) => {
  ...
  const handle = interceptor.post('/api/users', { id: 1 });
...
test('wait for async operations', async ({ mcp }) => {
  const interceptor = httpMock.interceptor();
  const handle = interceptor.get('/api/webhook', { received: true });
...
test('verify all mocks used', async ({ mcp }) => {
  ...
  interceptor.get('/api/users', [], { times: 1 });
  interceptor.get('/api/orders', [], { times: 1 });
...
test('with cleanup', async ({ mcp }) => {
  ...
  try {
    interceptor.get('/api/data', { result: true });
...
test('test 1', async ({ mcp }) => {
  interceptor.get('/api/data', { test: 1 });
  // ...
});

test('test 2', async ({ mcp }) => {
  interceptor.get('/api/data', { test: 2 });
  // ...
});
```
