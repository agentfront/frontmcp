...
<ParamField path="securitySchemesInInput" type="string[]">
  Names of security schemes that should remain in the tool input schema so callers supply those credentials. Combine with `authProviderMapper` to keep the rest resolved from user context. See [Strategy 6: Hybrid Security Inputs](#strategy-6-hybrid-security-inputs-per-scheme-control).
</ParamField>
...
### Strategy 6: Hybrid Security Inputs (Per-Scheme Control)

Best for: Hybrid workflows where some credentials must come from the end user (OAuth tokens) while others must stay in server-managed context (API keys, tenant secrets).
...
OpenapiAdapter.init({
  ...
  url: 'https://api.example.com/openapi.json',
  baseUrl: 'https://api.example.com',
  securitySchemesInInput: ['UserBearerAuth'],
  authProviderMapper: {
    ServiceApiKey: (authInfo) => authInfo.user?.apiKey,
  },
});
```
...
1. Only the schemes listed in `securitySchemesInInput` remain in the tool's input schema so callers can supply those credentials explicitly.
2. All other schemes are resolved from `authProviderMapper`, `securityResolver`, or the default auth context and never surface to MCP clients.
3. The adapter automatically removes filtered schemes from generated input schemas, preventing server secrets from leaking into prompts.

<Tip>
  Use this when endpoints need user-specific OAuth tokens alongside service accounts or tenant headers managed by your server.
</Tip>

<Warning>
  Schemes omitted from `securitySchemesInInput` must have matching entries in `authProviderMapper` or custom `securityResolver` logic; otherwise the adapter falls back to `authInfo.token`.
</Warning>
...
| **MEDIUM** ⚠️ | `staticAuth`, `additionalHeaders`, `securitySchemesInInput`, or default | Static credentials or hybrid security (partially user-provided) |
