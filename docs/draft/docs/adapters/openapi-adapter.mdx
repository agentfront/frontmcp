...
<ParamField path="headersMapper" type="(authInfo: AuthInfo, headers: Headers) => Headers">
  Function to dynamically set headers based on authenticated user context. Headers set here are hidden from MCP clients.
</ParamField>

<ParamField path="bodyMapper" type="(authInfo: AuthInfo, body: any) => any">
...
<ParamField path="securitySchemesInInput" type="string[]">
  Names of security schemes that should remain in the generated input schema. Pair with `authProviderMapper` to keep service credentials hidden while still prompting users for the schemes you list here.
</ParamField>
...
<Check>**Security Risk: LOW** — Full control over authentication resolution from user context.</Check>
...
### Hybrid Authentication: securitySchemesInInput (Medium Risk)

Best for: Mixing user-provided OAuth tokens with server-held secrets. Use when you want models to supply certain credentials but still keep service accounts hidden behind `authProviderMapper`.
...
OpenapiAdapter.init({
  ...
  url: 'https://api.example.com/openapi.json',
  baseUrl: 'https://api.example.com',
  // Only the listed schemes stay in the tool input schema
  securitySchemesInInput: ['UserToken'],
  // Every other scheme is resolved from context
  authProviderMapper: {
    ServiceKey: (authInfo) => authInfo.user?.serviceApiKey,
  },
});
```
...
<Info>
  Only schemes listed in `securitySchemesInInput` remain visible to MCP callers. All other schemes must be satisfied by `authProviderMapper`, `securityResolver`, or `staticAuth`.
</Info>
...
When the adapter runs inside a FrontMCP app it inherits the app logger via `setLogger()`. If you bootstrap adapters manually (CLI scripts, tests, workers), pass any `FrontMcpLogger` implementation so structured security diagnostics still flow to your sinks.
...
const baseLogger = pino({ name: 'openapi-adapter' });
const adapterLogger: FrontMcpLogger = {
  verbose: (msg, ...args) => baseLogger.debug({ args }, msg),
  debug: (msg, ...args) => baseLogger.debug({ args }, msg),
  info: (msg, ...args) => baseLogger.info({ args }, msg),
  warn: (msg, ...args) => baseLogger.warn({ args }, msg),
  error: (msg, ...args) => baseLogger.error({ args }, msg),
  child: (scope) => baseLogger.child({ scope }),
};
...
<Tip>
  Share a single logger instance across adapters so security warnings, transform diagnostics, and request metadata show up beside the rest of your platform logs.
</Tip>
...
<Card title="Expose Only Needed Schemes" icon="link">
  Use `securitySchemesInInput` to prompt for user-owned tokens while keeping server secrets in `authProviderMapper`. Document which schemes belong to which side so operators know what gets logged.
</Card>
...
| **MEDIUM** ⚠️ | `securitySchemesInInput` + `authProviderMapper` | Hybrid flows where some credentials come from user input while service secrets stay server-side |
