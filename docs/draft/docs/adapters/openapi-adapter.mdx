...
<Info>
  The `authInfo` parameter in `authProviderMapper`, `headersMapper`, and `bodyMapper`
  callbacks comes from the authenticated request context (`requestContext.authInfo`). By the time your tool executes,
  authentication has been verified and all auth fields are populated.
</Info>
