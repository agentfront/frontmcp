...
<ParamField path="logger" type="FrontMcpLogger" required>
  Logger instance for adapter diagnostics. When using `OpenapiAdapter.init()` within a FrontMCP app, the SDK automatically provides the logger. For standalone usage, you must provide a logger implementing the `FrontMcpLogger` interface.
</ParamField>
