...
## Session IDs & compatibility

FrontMcp now enforces a single context scope, so every request exposes a validated `sessionId` that you can safely reuse for caching, tracing, and logging.

### Validation rules

- Maximum length: **2048 characters**
- Allowed characters: letters, numbers, `- _ . :`
- Anonymous sessions are prefixed with `anon:`
- Whitespace and control characters are rejected before your code runs

```ts
const sessionId = this.context.sessionId;
const redisKey = `session:${sessionId}:recent`;
```

<Tip>
Need structured platform metadata? `ctx.authInfo.sessionIdPayload` now includes the detected `platformType` so you can branch on OpenAI vs Claude behavior without regexing the user agent.
</Tip>

### SessionKey helper (optional)

Legacy providers that depended on the `SessionProvider` API can inject `SessionKey` or `FrontMcpContext` directly now that everything is context-scoped:

```ts
import { Provider, ProviderScope, SessionKey } from '@frontmcp/sdk';

@Provider({ name: 'SessionPrefixedRedis', scope: ProviderScope.CONTEXT })
export class SessionPrefixedRedis {
  constructor(private readonly sessionKey: SessionKey) {}

  getPrefix() {
    return `session:${this.sessionKey.value}:`; // Validated when SessionKey is constructed
  }
}
```

> `SessionKey` simply wraps `ctx.sessionId` and throws if a consumer tries to use an invalid value. Migrating providers can swap to `ProviderScope.CONTEXT` and keep existing constructor signatures.

### Migrating legacy scopes

- `ProviderScope.SESSION` / `REQUEST` are automatically normalized to `ProviderScope.CONTEXT`, but updating the decorator removes the deprecation warnings.
- Replace `SessionProvider` dependencies with `FrontMcpContext` for richer metadata (auth info, transport handles, timing helpers).
- When deriving cache keys, keep the full session ID or hash it with SHA-256 to avoid collisions—FrontMCP already hashes IDs internally when logging.

<Warning>
If you truncate session IDs, keep at least the last 12 characters so hashes remain unique across transports.
</Warning>
