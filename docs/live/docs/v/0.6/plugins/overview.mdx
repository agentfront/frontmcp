...
<CardGroup cols={1}>
  ...
  <Card title="Approval Plugin" icon="shield-check" href="/docs/plugins/approval-plugin">
    Claude Code-style approvals keep sensitive tools gated behind PKCE/webhook security and scoped grants.

    **Features:**
    - Grant approvals per session, user, tool, or context with TTL-aware scopes.
    - PKCE-secured webhooks plus recheck mode to integrate third-party approval systems.
    - Audit trails record who granted or revoked each decision for compliance.
