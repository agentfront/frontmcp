...
<Tip>
  **Using a common provider?** Use presets for even simpler configuration:
  ```typescript
  auth: {
    mode: 'transparent',
...
| `preset`           | `'frontegg' \| 'auth0' \| 'okta'` | -   | Provider preset (alternative to provider) |
| `domain`           | `string`             | Required with preset | Provider domain for presets       |
...
| `advanced`         | `object`             | -               | Advanced options (scopes, dcrEnabled, endpoints) |

<Note>
  *Either `provider` or `preset`+`domain` is required.
</Note>

### Provider Examples

<Tabs>
  <Tab title="Frontegg (Preset)">
    ```typescript
    auth: {
      mode: 'transparent',
      preset: 'frontegg',
      domain: 'your-app.frontegg.com',
      clientId: 'your-client-id',
      expectedAudience: 'https://your-app.frontegg.com',
    }
    ```
  </Tab>
  <Tab title="Auth0 (Preset)">
    ```typescript
    auth: {
      mode: 'transparent',
      preset: 'auth0',
