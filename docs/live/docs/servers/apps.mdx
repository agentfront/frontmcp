...
<Note>
  Combine remote apps with tuned `transportOptions` (timeouts, retry attempts, and namespaces) and the server `transport` block (legacy SSE, streamable helpers, and session requirements) so orchestrator agents and gateway proxies keep cross-server flows predictable.
</Note>
