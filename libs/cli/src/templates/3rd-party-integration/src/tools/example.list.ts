import { z } from 'zod';
import { ExecuteContextSchema, HttpOutput, ToolExecute, makeHttpOutputSchema } from '../mcp-http-types.js';
...
export const execute: ToolExecute<Input, Body> = async ({ input, ctx }) => {
  ...
  if (res.status < 200 || res.status >= 300) {
    ...
    throw new Error(
      `Upstream error (${res.status}): ${errorBody.success ? errorBody.data.error.message : 'Unknown error'}`,
    );
  }
