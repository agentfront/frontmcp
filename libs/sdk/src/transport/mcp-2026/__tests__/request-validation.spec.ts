...
import { collectHeaderParams, isProtocol2026Request, readHeader, validate2026Request } from '../request-validation';

const META = {
  [MCP_2026_META.protocolVersion]: PROTOCOL_2026_07_28,
  [MCP_2026_META.clientCapabilities]: {},
};
...
describe('isProtocol2026Request', () => {
  it('claims a request declaring the version in _meta', () => {
    expect(isProtocol2026Request({ headers: {}, body: request('tools/list') })).toBe(true);
  });
  ...
  it('claims a request whose header names an unknown version', () => {
    ...
    expect(
      isProtocol2026Request({ headers: { 'mcp-protocol-version': '2099-01-01' }, body: { method: 'tools/list' } }),
    ).toBe(true);
  });

  it('claims 2026-only methods', () => {
    expect(isProtocol2026Request({ headers: {}, body: { method: 'server/discover' } })).toBe(true);
    expect(isProtocol2026Request({ headers: {}, body: { method: 'subscriptions/listen' } })).toBe(true);
  });
  ...
  it('does NOT claim a bare legacy initialize', () => {
    expect(isProtocol2026Request({ headers: {}, body: { method: 'initialize', params: {} } })).toBe(false);
  });
});

describe('validate2026Request', () => {
  ...
  it('rejects a missing protocol version header', () => {
    const result = validate2026Request({ headers: { 'mcp-method': 'tools/list' }, body: request('tools/list') });
    expect(result).toMatchObject({ ok: false, status: 400, error: { code: MCP_2026_ERROR_CODES.headerMismatch } });
  });
  ...
  it('rejects a body missing the _meta protocol version', () => {
    ...
    expect(result).toMatchObject({ ok: false, error: { code: MCP_2026_ERROR_CODES.headerMismatch } });
    expect((result as { error: { message: string } }).error.message).toContain(MCP_2026_META.protocolVersion);
  });
  ...
  it('reports an unsupported version with the supported list', () => {
    const body = {
      ...
      id: 1,
      method: 'tools/list',
      params: { _meta: { [MCP_2026_META.protocolVersion]: '2099-01-01' } },
    };
    const result = validate2026Request({
  ...
  it('rejects a missing Mcp-Method header', () => {
    const result = validate2026Request({ headers: headers(), body: request('tools/list') });
    expect(result).toMatchObject({ ok: false, error: { code: MCP_2026_ERROR_CODES.headerMismatch } });
  });
  ...
  it('validates Mcp-Name against params.name for tools/call', () => {
    ...
    expect(
      validate2026Request({ headers: headers({ 'mcp-method': 'tools/call', 'mcp-name': 'echo' }), body }),
    ).toMatchObject({ ok: true });

    expect(
      validate2026Request({ headers: headers({ 'mcp-method': 'tools/call', 'mcp-name': 'other' }), body }),
    ).toMatchObject({ ok: false, error: { code: MCP_2026_ERROR_CODES.headerMismatch } });

    expect(validate2026Request({ headers: headers({ 'mcp-method': 'tools/call' }), body })).toMatchObject({
      ok: false,
      error: { code: MCP_2026_ERROR_CODES.headerMismatch },
    });
  });
  ...
  it('rejects a body with no method', () => {
    const result = validate2026Request({ headers: headers(), body: { jsonrpc: '2.0', id: 1 } });
  ...
  describe('x-mcp-header parameters', () => {
    ...
    it('expects no header when the argument is absent', () => {
      expect(
        validate2026Request({ headers: headers(base), body: call({ query: 'x' }), lookupToolSchema }),
      ).toMatchObject({ ok: true });
    });
