...
async function resumeTask(params: {
  ...
  if (!runner) {
    scope.logger.warn('mcp-20260728: cannot resume task, no runner configured', { taskId: record.taskId });
    return;
  }
...
export function toJsonRpcError(error: unknown): { status: number; error: JsonRpcErrorPayload } {
  if (error instanceof MissingClientCapabilityError) {
    return {
      status: 400,
      error: {
        code: MCP_20260728_ERROR_CODES.missingRequiredClientCapability,
 ...
 */
export async function dispatch20260728(options: DispatchOptions): Promise<DispatchResult> {
  const {
    scope,
    body,
    clientCapabilities,
  ...
  if ((MCP_20260728_REMOVED_METHODS as readonly string[]).includes(method)) {
    return {
      kind: 'error',
      status: 404,
