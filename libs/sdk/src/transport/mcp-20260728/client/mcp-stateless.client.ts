 ...
 *   via `tasks/update`.
 *
 * @module transport/mcp-20260728/client
 */
...
export interface McpStatelessInputHandlers {
...
export interface McpStatelessClientOptions {
  ...
  handlers?: McpStatelessInputHandlers;
...
export class McpStatelessError extends Error {
  constructor(
...
const DEFAULT_CLIENT_INFO: Implementation = { name: 'frontmcp-20260728-client', version: '1.0.0' };

export class McpStatelessClient {
  private nextId = 1;
  private readonly options: Required<Pick<McpStatelessClientOptions, 'url' | 'maxInputRounds'>> &
    McpStatelessClientOptions;
  ...
  async request(method: string, params: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
    ...
    throw new McpStatelessError(-32603, `Server kept requesting input after ${this.options.maxInputRounds} rounds`);
  }
      ...
      switch (request.method) {
        case 'elicitation/create': {
          if (!handlers.onElicit)
            throw new McpStatelessError(-32603, 'Server requested elicitation but no handler is configured');
          answers[key] = await handlers.onElicit(request.params ?? {});
          break;
        }
        case 'sampling/createMessage': {
          if (!handlers.onSample)
            throw new McpStatelessError(-32603, 'Server requested sampling but no handler is configured');
          answers[key] = await handlers.onSample(request.params ?? {});
          break;
        }
        case 'roots/list': {
          if (!handlers.onListRoots)
            throw new McpStatelessError(-32603, 'Server requested roots but no handler is configured');
          answers[key] = (await handlers.onListRoots()) as unknown as Record<string, unknown>;
          break;
        }
        default:
          throw new McpStatelessError(-32603, `Unsupported input request: ${request.method}`);
      }
    }
  ...
  private async awaitTask(task: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    while (Date.now() < deadline) {
      ...
      if (status === 'failed') {
        ...
        throw new McpStatelessError(error?.code ?? -32603, error?.message ?? 'Task failed');
      }
      if (status === 'cancelled') throw new McpStatelessError(-32603, `Task ${taskId} was cancelled`);
    ...
    throw new McpStatelessError(-32603, `Task ${taskId} did not settle within ${ttl}ms`);
  }
  ...
  async listen(
    ...
    if (!response.ok || !response.body) {
      throw new McpStatelessError(-32603, `subscriptions/listen failed with HTTP ${response.status}`);
    }
    ...
    void this.pumpSse(response.body, (message) => {
      ...
      .then(() => {
        ...
        if (!settled) {
          rejectAck?.(new McpStatelessError(-32603, 'subscriptions/listen stream closed before acknowledgement'));
        }
      })
      .catch((error: unknown) => {
        if (settled) return; // Aborted by close() after a successful ack — expected.
        rejectAck?.(
          error instanceof McpStatelessError
            ? error
            : new McpStatelessError(-32603, `subscriptions/listen stream failed: ${String(error)}`),
        );
      });
  ...
  private async send(method: string, params: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if (payload.error) {
      throw new McpStatelessError(payload.error.code, payload.error.message, payload.error.data);
    }
  ...
  private buildRequest(
    ...
    const meta: Record<string, unknown> = {
      ...
      [MCP_20260728_META.clientInfo]: this.options.clientInfo ?? DEFAULT_CLIENT_INFO,
      [MCP_20260728_META.clientCapabilities]: this.options.capabilities ?? {},
    };
    if (this.options.logLevel) meta[MCP_20260728_META.logLevel] = this.options.logLevel;
  ...
  private warn(message: string): void {
    console.warn(`[McpStatelessClient] ${message}`);
  }
}
