...
describe('Resource.esm()', () => {
  it('creates ResourceEsmTargetRecord with kind ESM', () => {
    const record = (Resource as any).esm('@acme/tools@^1.0.0', 'status') as ResourceEsmTargetRecord;
  ...
  it('parses unscoped specifier correctly', () => {
    const record = (Resource as any).esm('my-tools@latest', 'config') as ResourceEsmTargetRecord;
  ...
  it('creates different symbols for different targets', () => {
    const r1 = (Resource as any).esm('@acme/tools@^1.0.0', 'status') as ResourceEsmTargetRecord;
    const r2 = (Resource as any).esm('@acme/tools@^1.0.0', 'config') as ResourceEsmTargetRecord;
    expect(r1.provide).not.toBe(r2.provide);
  });

  it('passes options through', () => {
    const record = (Resource as any).esm('@acme/tools@^1.0.0', 'status', {
  ...
  it('throws on empty specifier', () => {
    expect(() => (Resource as any).esm('', 'status')).toThrow('Package specifier cannot be empty');
  });

  it('throws on invalid specifier', () => {
    expect(() => (Resource as any).esm('!!!', 'status')).toThrow('Invalid package specifier');
  });
});

describe('Resource.remote()', () => {
  it('creates ResourceRemoteRecord with kind REMOTE', () => {
    const record = (Resource as any).remote('https://api.example.com/mcp', 'config') as ResourceRemoteRecord;
  ...
  it('passes transportOptions and remoteAuth', () => {
    const record = (Resource as any).remote('https://api.example.com/mcp', 'config', {
