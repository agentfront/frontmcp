  ...
  return (target: Function) => {
    ...
    if (error) {
      ...
      if (formatted.providers) {
        throw new InvalidDecoratorMetadataError('Plugin', 'providers', JSON.stringify(formatted.providers, null, 2));
      }
      if (formatted.adapters) {
        throw new InvalidDecoratorMetadataError('Plugin', 'adapters', JSON.stringify(formatted.adapters, null, 2));
      }
      if (formatted.tools) {
        throw new InvalidDecoratorMetadataError('Plugin', 'tools', JSON.stringify(formatted.tools, null, 2));
      }
      if (formatted.resources) {
        throw new InvalidDecoratorMetadataError('Plugin', 'resources', JSON.stringify(formatted.resources, null, 2));
      }
      if (formatted.prompts) {
        throw new InvalidDecoratorMetadataError('Plugin', 'prompts', JSON.stringify(formatted.prompts, null, 2));
      }
      throw error;
    }
