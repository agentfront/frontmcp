...
describe('EncryptedRedisVault', () => {
  ...
  beforeEach(() => {
    ...
    vault.setEncryptionContext(encryptionKey, claims.jti);
  });

  afterEach(() => {
    vault.clearEncryptionContext();
    redis.clear();
  });
  ...
  describe('Encryption Context', () => {
    ...
    it('should work after setting encryption context', async () => {
      vault.setEncryptionContext(encryptionKey, claims.jti);
  ...
  describe('Data Encryption', () => {
    ...
    it('should fail decryption with wrong key', async () => {
      ...
      await vault.addAppCredential(entry.id, {
        appId: 'slack',
        providerId: 'slack-oauth',
        credential: {
          type: 'oauth',
      ...
      vault.setEncryptionContext(wrongKey, 'different-vault');

      // Should fail to decrypt
      await expect(vault.get(entry.id)).rejects.toThrow('Failed to load vault');
    });
  });
  ...
  describe('App Credentials', () => {
    it('should add and retrieve credentials', async () => {
      ...
      const credential = await vault.getCredential(entry.id, 'openai', 'openai-api');
    ...
    it('should remove credentials', async () => {
      ...
      const credential = await vault.getCredential(entry.id, 'slack', 'slack-oauth');
    ...
    it('should get all credentials for an app', async () => {
      ...
      const credentials = await vault.getAppCredentials(entry.id, 'slack');
  ...
  describe('App Authorization', () => {
    it('should check app authorization without decryption', async () => {
      ...
      const isAuthorized = await vault.isAppAuthorized(entry.id, 'slack');
      expect(isAuthorized).toBe(true);

      const notAuthorized = await vault.isAppAuthorized(entry.id, 'github');
  ...
  describe('Zero Knowledge Properties', () => {
    ...
    it('should produce different encrypted data for same credentials', async () => {
      ...
      await vault.addAppCredential(entry1.id, credential);
      await vault.addAppCredential(entry2.id, credential);
