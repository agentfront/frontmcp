...
export class WorkerMemoryError extends WorkerPoolError {
  ...
  constructor(memoryBytes: number, limitBytes: number) {
    super(
      `Worker exceeded memory limit: ${Math.round(memoryBytes / 1024 / 1024)}MB > ${Math.round(limitBytes / 1024 / 1024)}MB`,
    );
...
export class MessageSizeError extends WorkerPoolError {
  ...
  constructor(sizeBytes: number, maxBytes: number) {
    super(
      `Message size ${Math.round(sizeBytes / 1024)}KB exceeds limit ${Math.round(maxBytes / 1024)}KB`,
    );
