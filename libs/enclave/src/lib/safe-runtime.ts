 ...
 */
export function createSafeRuntime(
  context: ExecutionContext,
  options?: SafeRuntimeOptions,
) {
  const { config, stats } = context;
  const sidecar = options?.sidecar;
  const referenceConfig = options?.referenceConfig;
  const resolver =
    sidecar && referenceConfig ? new ReferenceResolver(sidecar, referenceConfig) : undefined;
    ...
    if (resolver && resolver.containsReferences(args)) {
      ...
      try {
        ...
        if (error instanceof ResolutionLimitError) {
          throw new Error(
            `Failed to resolve references in tool arguments: ${error.message}`,
          );
        }
        throw error;
      }
