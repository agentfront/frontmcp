...
export class RuleBasedScorer extends BaseScorer {
  ...
  async score(features: ExtractedFeatures): Promise<ScoringResult> {
    ...
    const totalScore = this.clampScore(
  ...
  private checkSensitiveFields(
    ...
    signals.push({
      ...
      description: `Accesses sensitive fields: ${features.sensitive.fieldsAccessed.slice(0, 5).join(', ')}${features.sensitive.fieldsAccessed.length > 5 ? '...' : ''}`,
  ...
  private checkWildcardQueries(
    ...
    for (const tc of features.toolCalls) {
      ...
      if (
        tc.argumentKeys.length === 0 &&
        /query|search|find|filter/i.test(tc.toolName)
      ) {
        foundWildcard = true;
        break;
      }
   ...
   */
  private checkExtremeValues(
    features: ExtractedFeatures,
    signals: RiskSignal[]
  ): void {
    // Check all tool calls for extreme values
    for (const tc of features.toolCalls) {
      const extremeValues = tc.numericLiterals.filter(
        n => n > RULE_THRESHOLDS.EXTREME_VALUE
      );
   ...
   */
  private checkBulkOperations(
    features: ExtractedFeatures,
    signals: RiskSignal[]
  ): void {
    const bulkTools = features.toolCalls.filter(tc =>
      FeatureExtractor.isBulkOperation(tc.toolName)
    );
