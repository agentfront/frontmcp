...
export class FeatureExtractor {
  ...
  extract(code: string): ExtractedFeatures {
    ...
    walk.ancestor(ast, {
      CallExpression: (node: acorn.Node, _state: unknown, ancestors: acorn.Node[]) => {
        ...
        this.extractToolCall(node, loopDepth, toolCalls, toolSequence, allNumericLiterals, allStringLiterals, sensitiveFields, sensitiveCategories);
      },
      ...
      VariableDeclarator: (node: acorn.Node) => {
        ...
        if (
          ...
          varNode.init?.type === 'AwaitExpression'
        ) {
          // Check if it's a callTool result
          const awaitArg = (varNode.init as unknown as { argument?: { type?: string; callee?: { name?: string } } }).argument;
  ...
  private findMaxLimitValue(
    ...
    for (const tc of toolCalls) {
      const hasLimitArg = tc.argumentKeys.some(key =>
        LIMIT_FIELD_NAMES.includes(key.toLowerCase())
      );
