...
describe('Safe Runtime Reference Integration', () => {
  ...
  const createContext = (
    toolHandler?: ExecutionContext['toolHandler'],
  ): ExecutionContext => ({
    config: {
      timeout: 30000,
  ...
  describe('__safe_concat', () => {
    describe('without sidecar', () => {
      ...
      it('should throw if reference ID detected without sidecar', () => {
        ...
        expect(() => runtime.__safe_concat(refId, ' extra')).toThrow(
          'reference system not configured',
        );
        expect(() => runtime.__safe_concat('prefix ', refId)).toThrow(
          'reference system not configured',
        );
      });
    });
    ...
    describe('with sidecar (composites disabled)', () => {
      it('should throw when concatenating with reference ID', () => {
        ...
        expect(() => runtime.__safe_concat(refId, ' extra')).toThrow(
          'Composite handles are disabled',
        );
  ...
  describe('__safe_template', () => {
    describe('without sidecar', () => {
      ...
      it('should throw if reference ID detected without sidecar', () => {
        ...
        expect(() => runtime.__safe_template(['Result: ', ''], refId)).toThrow(
          'reference system not configured',
        );
      });
    });
