    ...
    it('should maintain isolation under concurrent load', async () => {
      ...
      const operations = [
        ...Array(5).fill(null).map(() =>
          tenant1.run(`const r = await callTool('increment', {}); return r;`)
        ),
        ...Array(3).fill(null).map(() =>
          tenant2.run(`const r = await callTool('increment', {}); return r;`)
        ),
        ...Array(7).fill(null).map(() =>
          tenant3.run(`const r = await callTool('increment', {}); return r;`)
        ),
      ];
