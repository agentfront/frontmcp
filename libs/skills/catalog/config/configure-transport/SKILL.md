...
- You only need to register tools, prompts, or resources (use `configure-server` instead)
- You are configuring authentication or session tokens (use `configure-auth` instead)
- You need to set up plugin middleware without changing the transport layer (use `configure-plugins` instead)
...
- **Related skills:** `configure-server`, `configure-auth`, `configure-plugins`
