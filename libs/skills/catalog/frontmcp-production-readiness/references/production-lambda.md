...
| [`sam-template`](../examples/production-lambda/sam-template.md)                               | Basic        | > Configuration authoring lives in **`frontmcp-deployment` → `references/deploy-to-lambda.md`**. This file is checklist-only: it verifies the SAM artifact pairs correctly with the bundle that `frontmcp build --target lambda` produces. |
