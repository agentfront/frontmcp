...
| `hideFromDiscovery` | `boolean`                                                                      | `false` | Hide from `tools/list` but still callable. Use for internal / agent-only tools.                                                                             |
...
- **`availableWhen` + `hideFromDiscovery`** — `availableWhen` is a hard constraint (filtered AND blocked); `hideFromDiscovery` is a soft hide (filtered but still callable).
