...
The 30s `requestTimeout` applies when you pass no `signal` in the options; a `Request`'s own signal still aborts it. `this.fetch()` is also aborted with `this.signal`, so a cancelled or timed-out call stops its requests.
