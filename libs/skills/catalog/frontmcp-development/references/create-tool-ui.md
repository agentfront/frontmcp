...
> **Claude target reality check (issues #447 / #455).** Claude Desktop / claude.ai render widgets in a sandboxed iframe whose CSP blocks **all** external `<script src="https://…">` execution — including `cdnjs.cloudflare.com`. Setting `_meta.ui.csp` on the tool doesn't help either — Claude only honors CSP declared on the **resource** (`resources/read` content `_meta`), which FrontMCP doesn't emit today (tracked in #455).
>
> **Set `resourceMode: 'inline'` for `.tsx`/`.jsx` FileSource widgets you want to render in Claude (fixed in #454).** With `resourceMode: 'inline'` the bundler inlines `react` / `react-dom` into the widget's `<script type="module">` — no esm.sh import map, no external module — so the widget is fully self-contained and runs under Claude's default CSP. The default `'cdn'` mode still uses esm.sh and is the right pick for OpenAI Apps SDK / ChatGPT / Cursor / MCP Inspector (smaller payload). If you don't want React at all, a `uiType: 'html'` function template with a single inline `<script>` block also works on every host.
...
| **Claude (MCP-UI)**  | `inline` (dual-payload) | postMessage       | No external script execution in widget iframe (CSP blocks esm.sh AND cdnjs). For `.tsx` FileSource widgets, set `resourceMode: 'inline'` so React is bundled into the widget (#454 fix). For non-React widgets, use a self-contained `uiType: 'html'` template. CSP-on-tool ignored, see #455. |
