...
> **Claude target reality check (issues #447 / #454 / #455).** Claude Desktop / claude.ai render widgets in a sandboxed iframe whose CSP blocks **all** external `<script src="https://…">` execution — including `cdnjs.cloudflare.com`. Even with `resourceMode: 'inline'`, the user's component code is still served via an esm.sh import map for `react` / `react-dom/client` / `react/jsx-runtime`, so `.tsx` FileSource widgets currently hang on the "Loading widget…" placeholder forever in Claude. Setting `_meta.ui.csp` on the tool doesn't help either — Claude only honors CSP declared on the **resource** (`resources/read` content `_meta`), which FrontMCP doesn't emit today (#455). For Claude targets, use a fully self-contained `uiType: 'html'` function template that emits a single inline `<script>` block — no import map, no external module, no esm.sh. The same widget renders correctly in OpenAI Apps SDK, ChatGPT, and MCP Inspector.
...
| **Claude (MCP-UI)**  | `inline` (dual-payload) | postMessage       | **No external script execution** in widget iframe — including esm.sh AND cdnjs; only fully self-contained inline HTML renders (see #447 / #454 / #455). `.tsx` FileSource currently hangs on "Loading widget…" |
...
| Claude-targeted widgets   | Self-contained `uiType: 'html'` template (single inline `<script>`, no import map) | `.tsx`/`.jsx` FileSource (CDN-loaded React) — even with `resourceMode: 'inline'` | Claude blocks ALL external script execution including esm.sh and cdnjs; FileSource widgets hang on "Loading widget…" forever (see #454) |
...
| "Loading widget…" hangs forever in Claude (FileSource) | Claude blocks ALL external script execution in the widget iframe; the auto-emitted esm.sh import map (and `<script type="module">`) for React never runs (#447 / #454) | Switch to a fully self-contained `uiType: 'html'` function template that emits a single inline `<script>` block — no import map, no `<script src>`. Pinning to `cdnjs.cloudflare.com` does NOT help, contrary to older docs. |
