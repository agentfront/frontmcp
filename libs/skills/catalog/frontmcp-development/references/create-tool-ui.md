...
| **Claude (MCP-UI)**  | `inline` (dual-payload) | postMessage       | **Only `cdnjs.cloudflare.com`** is reachable; prefer `resourceMode: 'inline'` for self-contained widgets |
...
| Claude-targeted widgets   | `resourceMode: 'inline'` + CDN deps on `cdnjs.cloudflare.com`         | Default `'cdn'` + arbitrary CDN domains        | Claude blocks all non-cdnjs origins; widget will hang forever      |
