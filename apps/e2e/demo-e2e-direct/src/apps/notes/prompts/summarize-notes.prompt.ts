...
export default class SummarizeNotesPrompt extends PromptContext {
  async execute(args: Record<string, string>): Promise<GetPromptResult> {
    ...
    const systemContent =
      format === 'detailed'
        ? `You are a helpful assistant that summarizes notes in detail. Include key points and insights from each note.`
        : `You are a helpful assistant that provides brief summaries. Be concise.`;
