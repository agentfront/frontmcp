/**
 * Parallel Stress Tests for Config System (5 workers × 1000 iterations)
 *
 * Tests configuration operations under parallel load using multiple clients
 * to achieve higher throughput (400-2000+ req/s)
 */
import { perfTest, expect } from '@frontmcp/testing';

perfTest.describe('Config Parallel Stress Testing', () => {
  perfTest.use({
  ...
  perfTest('parallel stress: 5000 total get-config operations', async ({ perf, server }) => {
    ...
    console.log(
      `[PARALLEL] get-config: ${result.totalRequestsPerSecond.toFixed(1)} req/s total ` +
  ...
  perfTest('parallel stress: 5000 total get-all-config operations', async ({ perf, server }) => {
    const result = await perf.checkLeakParallel(
      (client) => async () => {
        await client.tools.call('get-all-config', {});
      },
      {
        iterations: 1000,
        workers: 5,
    ...
    console.log(
      `[PARALLEL] get-all-config: ${result.totalRequestsPerSecond.toFixed(1)} req/s total ` +
  ...
  perfTest('parallel stress: 5000 total mixed config operations', async ({ perf, server }) => {
    const result = await perf.checkLeakParallel(
      (client, workerId) => {
        ...
        return async () => {
          ...
          if (op === 0) {
            await client.tools.call('get-config', { key: 'NODE_ENV', defaultValue: 'test' });
          } else if (op === 1) {
            await client.tools.call('check-config', { key: 'PORT' });
          } else {
            await client.tools.call('get-all-config', {});
          }
        };
