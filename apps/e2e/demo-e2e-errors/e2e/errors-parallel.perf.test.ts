/**
 * Parallel Stress Tests for Errors System (5 workers × 1000 iterations)
 *
 * Tests error handling under parallel load using multiple clients
 */
import { perfTest, expect } from '@frontmcp/testing';

perfTest.describe('Errors Parallel Stress Testing', () => {
  perfTest.use({
  ...
  perfTest('parallel stress: 5000 total successful-tool operations', async ({ perf, server }) => {
    const result = await perf.checkLeakParallel(
      (client, workerId) => {
        let counter = workerId * 1000;
        return async () => {
          await client.tools.call('successful-tool', {
    ...
    console.log(
      `[PARALLEL] successful-tool: ${result.totalRequestsPerSecond.toFixed(1)} req/s total ` +
  ...
  perfTest('parallel stress: 5000 total error handling operations', async ({ perf, server }) => {
    const result = await perf.checkLeakParallel(
      (client, workerId) => {
        let counter = workerId * 1000;
        return async () => {
          try {
            await client.tools.call('throw-validation-error', {
              field: `field-${counter++}`,
            });
          } catch {
            // Expected to throw
    ...
    console.log(
      `[PARALLEL] error handling: ${result.totalRequestsPerSecond.toFixed(1)} req/s total ` +
    ...
    const result = await perf.checkLeakParallel(
      (client, workerId) => {
        ...
        return async () => {
          ...
          if (op === 0) {
            await client.tools.call('successful-tool', { message: 'test' });
          } else {
            try {
              await client.tools.call('throw-validation-error', { field: 'test' });
            } catch {
              // Expected to throw
            }
          }
