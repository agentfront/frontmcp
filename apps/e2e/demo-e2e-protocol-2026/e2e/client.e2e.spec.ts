/**
 * `Mcp2026Client` — the FrontMCP client speaking 2026-07-28 end to end.
 *
...
test.describe('protocol 2026-07-28 — Mcp2026Client', () => {
  ...
  const client = (server: { info: { baseUrl: string } }, overrides = {}) =>
    new Mcp2026Client({ url: server.info.baseUrl, ...overrides });
  ...
  test('surfaces a server error as Mcp2026Error with its JSON-RPC code', async ({ server }) => {
    const mcp = client(server);
  ...
  test('exposes Mcp2026Error for unsupported protocol versions', () => {
    ...
    const error = new Mcp2026Error(-32022, 'Unsupported protocol version: 2099-01-01', { supported: [] });
    expect(error.code).toBe(-32022);
    expect(error.name).toBe('Mcp2026Error');
  });
});
...
test.describe('protocol 2026-07-28 — remote-proxy adapter', () => {
  ...
  test('presents the remote through the Client-shaped surface', async ({ server }) => {
    const adapter = new Mcp2026ClientAdapter({ url: server.info.baseUrl });
