...
import { mcp2026Fetch, META_SERVER_INFO, PROTOCOL_2026 } from './helpers/mcp-2026-client';
...
test.describe('protocol 2026-07-28 — server/discover', () => {
  ...
  test('responds to server/discover without any prior handshake', async ({ server }) => {
    const res = await mcp2026Fetch(server.info.baseUrl, { method: 'server/discover', id: 1 });
  ...
  test('advertises 2026-07-28 among supportedVersions', async ({ server }) => {
    const res = await mcp2026Fetch(server.info.baseUrl, { method: 'server/discover', id: 2 });
    const { result } = res.json();

    expect(Array.isArray(result.supportedVersions)).toBe(true);
    expect(result.supportedVersions).toContain(PROTOCOL_2026);
  });

  test('still advertises the legacy versions it supports', async ({ server }) => {
    const res = await mcp2026Fetch(server.info.baseUrl, { method: 'server/discover', id: 3 });
    const { result } = res.json();
  ...
  test('returns server capabilities and instructions', async ({ server }) => {
    const res = await mcp2026Fetch(server.info.baseUrl, { method: 'server/discover', id: 4 });
    const { result } = res.json();
  ...
  test('carries resultType "complete" and serverInfo in _meta', async ({ server }) => {
    const res = await mcp2026Fetch(server.info.baseUrl, { method: 'server/discover', id: 5 });
    const { result } = res.json();
  ...
  test('is cacheable — carries ttlMs and cacheScope', async ({ server }) => {
    const res = await mcp2026Fetch(server.info.baseUrl, { method: 'server/discover', id: 6 });
    const { result } = res.json();
  ...
  test('declares the extensions field on capabilities', async ({ server }) => {
    const res = await mcp2026Fetch(server.info.baseUrl, { method: 'server/discover', id: 7 });
    const { result } = res.json();
