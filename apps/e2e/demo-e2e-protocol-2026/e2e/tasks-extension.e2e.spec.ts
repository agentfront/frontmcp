...
test.describe('protocol 2026-07-28 — tasks extension', () => {
  test.use({
    server: 'apps/e2e/demo-e2e-protocol-2026/src/main-tasks.ts',
  ...
  test('polls to completion via tasks/get and carries the result', async ({ server, auth }) => {
    ...
    const created = await mcp2026Fetch(server.info.baseUrl, {
      method: 'tools/call',
      id: 5,
  ...
  test('cancels a task', async ({ server, auth }) => {
    ...
    const cancelled = await mcp2026Fetch(server.info.baseUrl, {
      method: 'tasks/cancel',
      id: 13,
      params: { taskId },
      clientCapabilities: TASKS_EXT,
  ...
  test.describe('mid-flight input', () => {
    ...
    test('resumes and completes after tasks/update', async ({ server, auth }) => {
      ...
      const updated = await mcp2026Fetch(server.info.baseUrl, {
        method: 'tasks/update',
        id: 16,
