...
export interface Mcp2026CallOptions {
...
export function buildMcp2026Request(opts: Mcp2026CallOptions): {
  body: JsonRpcRequestBody;
  headers: Record<string, string>;
} {
  const protocolVersion = opts.protocolVersion ?? PROTOCOL_2026;
...
export async function mcp2026Fetch(baseUrl: string, opts: Mcp2026CallOptions): Promise<Mcp2026Response> {
  const { body, headers } = buildMcp2026Request(opts);
 ...
 */
export async function openMcp2026Stream(baseUrl: string, opts: Mcp2026CallOptions): Promise<SseStreamHandle> {
  const { body, headers } = buildMcp2026Request(opts);
