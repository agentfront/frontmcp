...
test.describe('protocol 2026-07-28 — stateless requests', () => {
  ...
  test('tools/list works with no initialize and no session', async ({ server }) => {
    const res = await mcpStatelessFetch(server.info.baseUrl, { method: 'tools/list', id: 1 });
  ...
  test('never mints an Mcp-Session-Id', async ({ server }) => {
    const res = await mcpStatelessFetch(server.info.baseUrl, { method: 'tools/list', id: 2 });
  ...
  test('every result carries resultType "complete"', async ({ server }) => {
    ...
    for (const [i, call] of calls.entries()) {
      const res = await mcpStatelessFetch(server.info.baseUrl, { ...call, id: 100 + i });
      const body = res.json();
  ...
  test('every result carries serverInfo in _meta', async ({ server }) => {
    ...
    for (const [i, method] of methods.entries()) {
      const res = await mcpStatelessFetch(server.info.baseUrl, { method, id: 200 + i });
  ...
  test('does not infer capabilities from a previous request', async ({ server }) => {
    ...
    await mcpStatelessFetch(server.info.baseUrl, {
      method: 'tools/list',
      id: 6,
  ...
  test('returns tools/list in a deterministic order across calls', async ({ server }) => {
    const first = await mcpStatelessFetch(server.info.baseUrl, { method: 'tools/list', id: 8 });
    const second = await mcpStatelessFetch(server.info.baseUrl, { method: 'tools/list', id: 9 });

    const names = (r: McpStatelessResponse) => (r.json().result.tools as ListedTool[]).map((t) => t.name);
