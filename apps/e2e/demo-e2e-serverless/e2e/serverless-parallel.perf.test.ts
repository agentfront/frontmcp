/**
 * Parallel Stress Tests for Serverless System (5 workers × 1000 iterations)
 *
 * Tests serverless operations under parallel load using multiple clients
 */
import { perfTest, expect } from '@frontmcp/testing';

perfTest.describe('Serverless Parallel Stress Testing', () => {
  perfTest.use({
  ...
  perfTest('parallel stress: 5000 total serverless-info operations', async ({ perf, server }) => {
    const result = await perf.checkLeakParallel(
      (client) => async () => {
        await client.tools.call('serverless-info', {});
      },
      {
        iterations: 1000,
        workers: 5,
    ...
    console.log(
      `[PARALLEL] serverless-info: ${result.totalRequestsPerSecond.toFixed(1)} req/s total ` +
  ...
  perfTest('parallel stress: 5000 total mixed serverless operations', async ({ perf, server }) => {
    const result = await perf.checkLeakParallel(
      (client, workerId) => {
        ...
        return async () => {
          ...
          if (op === 0) {
            ...
            await client.tools.call('cold-start-test', {});
          }
        };
      },
      {
