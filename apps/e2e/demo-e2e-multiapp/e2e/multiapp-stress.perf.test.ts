/**
 * Sequential Stress Tests for MultiApp System (1000 iterations)
 *
 * Tests multi-app isolation under sustained sequential load
 */
import { perfTest, expect } from '@frontmcp/testing';

perfTest.describe('MultiApp Sequential Stress Testing', () => {
  perfTest.use({
  ...
  perfTest('stress test: 1000 create-task operations', async ({ mcp, perf }) => {
    let counter = 0;
    const result = await perf.checkLeak(
      async () => {
    ...
    console.log(
      `[STRESS] create-task: ${result.requestsPerSecond?.toFixed(1)} req/s, ${result.samples.length} iterations`,
    );

    expect(result.growthRate).toBeLessThan(200 * 1024);
  });

  perfTest('stress test: 1000 mixed multi-app operations', async ({ mcp, perf }) => {
    let callIndex = 0;
    const result = await perf.checkLeak(
      async () => {
        ...
        if (op === 0) {
          ...
          await mcp.tools.call('create-task', { title: `Task ${callIndex}`, description: `Desc ${callIndex}` });
        } else if (op === 3) {
          await mcp.tools.call('list-tasks', {});
        } else if (op === 4) {
          await mcp.tools.call('create-event', { title: `Event ${callIndex}`, date: new Date().toISOString() });
        } else {
          await mcp.tools.call('list-events', {});
        }
      },
      {
        iterations: 1000,
