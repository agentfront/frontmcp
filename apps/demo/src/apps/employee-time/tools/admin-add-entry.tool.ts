...
@Tool({
  ...
  outputSchema: { result: z.object({ employeeId: z.string(), startedAt: z.number() }) as any }
})
export default class AdminAddEntryTool extends ToolContext {
  async execute(input: { admin: boolean, employeeId: string, siteId: string, at: number | string }): Promise<{ result: { employeeId: string, startedAt: number } }> {
    ...
    return { result: { employeeId, startedAt } };
  }
}
